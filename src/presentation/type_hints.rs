                ...
                if self.settings.show_return_types {
                    if let Some(func_sig) = type_context.functions.get(name) {
                        if self.should_show_hint(&func_sig.return_type) {
                            hints.push(self.create_return_type_hint(
                                name,
            ...
            Expression::Call { function, args } => {
                ...
                if let Expression::Identifier(func_name) = &**function {
                    if let Some(func_sig) = type_context.functions.get(func_name) {
                        if self.settings.show_return_types
                            && self.should_show_hint(&func_sig.return_type)
                        {
    ...
    fn format_type_hint(&self, type_res: &TypeResolution) -> String {
        let base_text = match &type_res.result {
            ResolutionResult::Concrete(ConcreteType::Primitive(primitive)) => match primitive {
                crate::domain::types::PrimitiveType::String => "Строка",
                crate::domain::types::PrimitiveType::Number => "Число",
                crate::domain::types::PrimitiveType::Boolean => "Булево",
                crate::domain::types::PrimitiveType::Date => "Дата",
            }
            .to_string(),
    ...
    fn format_concrete_type(&self, concrete_type: &ConcreteType) -> String {
        match concrete_type {
            ConcreteType::Primitive(primitive) => match primitive {
                crate::domain::types::PrimitiveType::String => "Str",
                crate::domain::types::PrimitiveType::Number => "Num",
                crate::domain::types::PrimitiveType::Boolean => "Bool",
                crate::domain::types::PrimitiveType::Date => "Date",
            }
            .to_string(),
...
impl SemanticTokensProvider {
    ...
    fn create_function_token(
        &self,
        func_name: &str,
        func_sig: &crate::domain::analysis::type_checker::FunctionSignature,
        line: u32,
        start_char: u32,
    ) -> SemanticToken {
        SemanticToken {
...
mod tests {
    ...
    fn create_test_context() -> TypeContext {
        ...
        functions.insert(
            ...
            crate::domain::analysis::type_checker::FunctionSignature {
                ...
                return_type: crate::domain::standard_types::primitive_type(
                    crate::domain::types::PrimitiveType::Number,
                ),
                exported: true,
            },
    ...
    fn test_format_type_hint() {
        ...
        let string_type = crate::domain::standard_types::primitive_type(
            crate::domain::types::PrimitiveType::String,
        );
        ...
        // Тест с Union типом
        let union_type =
            crate::domain::analysis::union_types::UnionTypeManager::from_concrete_types(vec![
                ConcreteType::Primitive(crate::domain::types::PrimitiveType::String),
                ConcreteType::Primitive(crate::domain::types::PrimitiveType::Number),
            ]);
