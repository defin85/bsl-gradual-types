...
4. `backend`, `web`, `bsl-agent` и `cli` остались transport wrappers над shared runtime contract и общей bounded observability taxonomy.
...
- shipped smoke entry point прогоняет `./scripts/run-intellisense-tests.sh smoke` до perf-gate summary и OpenSpec validate;
- docs не описывают legacy AST-based hover/completion fallback как production path;
- web module-context evidence считается только для path-aware surfaces: текущий inline hover API использует synthetic path `hover_request.bsl`, поэтому не засчитывается как доказательство module-context wiring без изменения API;
