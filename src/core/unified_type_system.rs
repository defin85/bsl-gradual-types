...
use super::types::{TypeResolution, Certainty, ResolutionResult, ConcreteType, ResolutionSource, FacetKind};
use super::platform_resolver::{PlatformTypeResolver, CompletionItem};
...
impl UnifiedTypeSystem {
    ...
    pub async fn get_all_type_resolutions(&self) -> Vec<(String, TypeResolution)> {
        ...
        resolutions.iter().map(|(k, v)| (k.clone(), v.clone())).collect()
    }
    ...
    async fn load_configuration_types(&self, config_path: &str) -> Result<()> {
        ...
        for config_resolution in config_resolutions {
            if let ResolutionResult::Concrete(ConcreteType::Configuration(config)) = &config_resolution.result {
        ...
        *self.configuration_parser.write().await = Some(guided_parser);
        ...
        println!("✅ Загружено {} конфигурационных типов", stats.configuration_types_count);
        Ok(())
    }
    ...
    async fn cache_resolution(&self, expression: &str, resolution: &TypeResolution) {
        ...
        cache.insert(expression.to_string(), CachedResolution {
...
impl LspTypeInterface {
    ...
    pub async fn check_assignment_compatibility(&self, _from_type: &TypeResolution, _to_type: &TypeResolution) -> bool {
...
impl WebTypeInterface {
    ...
    pub async fn search_types(&self, query: &str) -> Vec<TypeDisplayInfo> {
        ...
        matching_resolutions
            .into_iter()
            .enumerate()
            .map(|(i, resolution)| TypeDisplayInfo::from_resolution(format!("search_result_{}", i), resolution))
            .collect()
    }
    ...
    pub async fn get_type_details(&self, type_id: &str) -> Option<TypeDetailedInfo> {
        if let Some(resolution) = self.unified_system.get_type_by_id(type_id).await {
            Some(TypeDetailedInfo::from_resolution(type_id.to_string(), resolution))
        } else {
            None
        }
    }
...
impl TypeDetailedInfo {
    pub fn from_resolution(id: String, resolution: TypeResolution) -> Self {
        let name = match &resolution.result {
            ResolutionResult::Concrete(ConcreteType::Platform(platform_type)) => platform_type.name.clone(),
            ResolutionResult::Concrete(ConcreteType::Configuration(config_type)) => config_type.name.clone(),
            _ => "Unknown".to_string()
        };
        
        let methods = match &resolution.result {
            ResolutionResult::Concrete(ConcreteType::Platform(platform_type)) => {
                platform_type.methods.iter().map(|m| m.name.clone()).collect()
            }
            ResolutionResult::Concrete(ConcreteType::Configuration(config_type)) => {
                config_type.attributes.iter().map(|a| a.name.clone()).collect()
            }
            _ => Vec::new()
        };
        ...
        let properties = match &resolution.result {
            ResolutionResult::Concrete(ConcreteType::Platform(platform_type)) => {
                platform_type.properties.iter().map(|p| p.name.clone()).collect()
            }
            _ => Vec::new()
        };
