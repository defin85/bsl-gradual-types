...
- [x] 2.2 Зафиксировать и реализовать bounded strict priority lanes `control -> completion -> general`, включая явную queue/backpressure policy: reserved progress для `control`, bounded completion admission без silent drop, preserved current-revision handoff для `didOpen/didChange/didSave/didClose` и `general` как primary backpressure lane для unrelated traffic; weighted/fair scheduler в этот change не входит.
...
- [x] 3.1 Добавить backend tests на то, что `documentSymbol`/general backlog больше не задерживает completion в окне `adapter read -> dispatch`, не ломает current-revision handoff для `didChange` на default path, и что scheduler owner остаётся единственной точкой `poll_ready()/call()`.
