...
suite('Providers Test Suite', () => {
    ...
    test('BslOverviewProvider should be registered', async () => {
        ...
        assert.ok(commands.includes('bslAnalyzer.refreshOverview'), 'Overview refresh command should exist');
    });
    test('BslOverviewProvider refresh should work', async function () {
        ...
        try {
            ...
            assert.ok(error.message.includes('not found') === false, 'Command should be registered');
        }
    });
    /**
    ...
    test('BslDiagnosticsProvider should be registered', async () => {
        const commands = await vscode.commands.getCommands();
        assert.ok(commands.includes('bslAnalyzer.refreshDiagnostics'), 'Diagnostics refresh command should exist');
    });
    ...
    test('HierarchicalTypeIndexProvider should be registered', async () => {
        const commands = await vscode.commands.getCommands();
        assert.ok(commands.includes('bslAnalyzer.refreshTypeIndex'), 'Type index refresh command should exist');
    });
    ...
    test('BslPlatformDocsProvider should be registered', async () => {
        const commands = await vscode.commands.getCommands();
        assert.ok(commands.includes('bslAnalyzer.refreshPlatformDocs'), 'Platform docs refresh command should exist');
        assert.ok(commands.includes('bslAnalyzer.addPlatformDocs'), 'Add platform docs command should exist');
        assert.ok(commands.includes('bslAnalyzer.removePlatformDocs'), 'Remove platform docs command should exist');
    });
...
suite('Provider LSP Integration Test Suite', () => {
    test('Providers should update when LSP sends diagnostics', async function () {
        ...
        const commands = await vscode.commands.getCommands();
        const refreshCommands = commands.filter(cmd => cmd.startsWith('bslAnalyzer.refresh'));
        assert.ok(refreshCommands.length >= 4, `Should have at least 4 refresh commands, found: ${refreshCommands.length}`);
    });
    test('Providers should handle LSP server restart', async function () {
        ...
        try {
            ...
            const providerCommandsBefore = commandsBefore.filter(cmd => cmd.includes('refresh'));
            ...
            const providerCommandsAfter = commandsAfter.filter(cmd => cmd.includes('refresh'));
            assert.strictEqual(providerCommandsAfter.length, providerCommandsBefore.length, 'Provider commands should remain after server restart');
        }
        catch (error) {
...
suite('Provider Performance Test Suite', () => {
    test('Provider refresh should be fast', async function () {
        ...
        try {
            ...
            assert.ok(elapsed < 2000, `Provider refresh took ${elapsed}ms (should be < 2000ms)`);
        }
        catch (error) {
            // Может не работать в тестовой среде
            this.skip();
    ...
    test('Multiple provider refreshes should work concurrently', async function () {
        ...
        try {
            ...
            assert.ok(elapsed < 5000, `Concurrent refresh took ${elapsed}ms (should be < 5000ms)`);
        }
        catch (error) {
            // Может не работать в тестовой среде
            this.skip();
