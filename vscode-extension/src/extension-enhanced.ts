...
// Providers
let typeHintsProvider: TypeHintsProvider;
let codeActionsProvider: CodeActionsProvider;
let diagnosticsProvider: EnhancedDiagnosticsProvider;
 ...
 */
async function initializeEnhancedLsp(): Promise<void> {
    outputChannel.appendLine('🔗 Initializing Enhanced LSP client...');
    
    // Получаем путь к enhanced LSP серверу
    const serverPath = getBinaryPath('lsp-server', extensionContext);
    ...
    languageClient = new EnhancedLspClient(
        serverPath,
        outputChannel,
        performanceMonitor
    );
 ...
 */
async function registerEnhancedProviders(): Promise<void> {
    ...
    typeHintsProvider = new TypeHintsProvider(languageClient);
    ...
    codeActionsProvider = new CodeActionsProvider(languageClient);
    extensionContext.subscriptions.push(
    ...
    diagnosticsProvider = new EnhancedDiagnosticsProvider(languageClient, outputChannel);
 ...
 */
function registerEnhancedCommands(): void {
 ...
 */
async function showTypeInfoAtCursor(): Promise<void> {
 ...
 */
async function runPerformanceProfiling(): Promise<void> {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
 ...
 */
async function showProjectAnalysisResults(results: any): Promise<void> {
    const panel = vscode.window.createWebviewPanel(
        'bslProjectResults',
 ...
 */
function generateTypeInfoHtml(hover: any): string {
    return `
        <!DOCTYPE html>
        <html>
        <head>
