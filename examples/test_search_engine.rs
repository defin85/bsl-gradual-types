...
async fn main() -> Result<()> {
    ...
    if types_count > 0 {
        ...
        let config_provider =
            bsl_gradual_types::documentation::ConfigurationDocumentationProvider::new();
        search_engine
        ...
        println!(
            "Предложения для 'Табли': {:?}",
            suggestions.iter().take(5).collect::<Vec<_>>()
        );
        ...
        println!(
            "Среднее время поиска: {:.2}ms",
            stats.average_search_time_ms
        );
    } else {
        println!("⚠️ Нет загруженных типов для демонстрации поиска");
        println!(
            "   Убедитесь, что есть файлы справки синтакс-помощника в examples/syntax_helper/"
        );
    }
