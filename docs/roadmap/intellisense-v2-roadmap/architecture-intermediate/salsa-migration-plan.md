...
## 13. Открытые вопросы (закрыто для P5)

Эти вопросы **должны быть закрыты до P5**, иначе высокий риск mixed state/mixed deps и недетерминизма.
Ниже — фиксируем ответы, опираясь на текущее состояние репозитория.

### 13.1 Какой salsa crate выбираем и почему

**Решение (для P5):** используем `salsa` crate.

- Уже выбран в workspace: `Cargo.toml` содержит `salsa = "0.25.2"`.
- Уже используется в v2 ядре: `analysis-v2/src/lib.rs` построен на `#[salsa::input]`/`#[salsa::tracked]`.
- Нужные свойства для P5 есть прямо сейчас: cancellation через `salsa::Cancelled` и чистые queries.

**Почему не `ra_ap_salsa` сейчас:** мы не используем rust-analyzer crates напрямую (у нас tree-sitter + свой IR),
а `ra_ap_*` обычно имеет смысл, когда нужно совпадать с версионированием/экосистемой rust-analyzer.
Если позже появится явная потребность использовать ra crates — вернёмся к оценке стоимости миграции.

### 13.2 Представление текста в DB: `String/Arc<str>` vs `Rope`

**Решение (для P5):** хранить текст как `Arc<str>` в salsa input.

- Текущее состояние: `analysis-v2::SourceFile.text: Arc<str>`.
- `line_index(FileId)` и `parse_result(FileId)` уже считаются от цельного текста (простая корректность).
- В LSP `didChange` сейчас всё равно формирует новый полный текст (применяя `contentChanges`) и делает `SetFile`.

**Почему не Rope сейчас:** добавит сложность в API (срезы, конвертация в `String` для tree-sitter/IR),
а выигрыш без профилирования не гарантирован. Возвращаемся к Rope только если метрики покажут,
что стоимость пересборки `LineIndex/parse_result` от цельного текста доминирует по latency/памяти.

### 13.3 Граница "deps": что внутри `DepsSnapshot`, что может быть вне DB

**Определение:** deps = всё, что может измениться независимо от `file_text` и влияет на семантику/результаты LSP.

**Решение (минимум для P5):**

- Внутри `DepsSnapshot` (salsa input) держим только **snapshot-safe** семантические зависимости,
  которые читаются queries: `TypeRepository`, `TypeResolver`, `SignatureIndex` (см. `analysis-v2::SemanticDeps`).
- Вне DB допускаются сервисы/кэши/индексы, но только при выполнении правил:
  - **не читать их внутри salsa queries**;
  - **не мутировать in-place** объекты, которые уже попали в снапшот (любое изменение → новый снапшот + новый `deps_id`);
  - в LSP hot path брать **request-local clone snapshot** (например, `IndexSnapshot`) и не переиспользовать mutable ссылки;
  - observed ids (`deps_id/settings_id/file_version`) логируются для диагностики.

Практически для P5 это означает:
- IR (query `ir(FileId)`) зависит от `deps_id()` и использует deps только из `DepsSnapshot`.
- completion/hover/signatureHelp в v2 пути не берут `AnalysisEngine/Repository` “сбоку”,
  а используют deps из v2 снапшота (иначе возможен mixed deps).

### 13.4 Инкрементальность tree-sitter: где живёт и как не сломать snapshot-safety

**Решение (для P5):** `parse_result(FileId)` в v2 — чистая функция от `file_text` и `settings_id`.

- В v2 пути не используем tree-sitter `InputEdit`/инкрементальные деревья как источник correctness.
- Инкрементальный парсинг (если нужен по производительности) может остаться legacy-оптимизацией вне DB
  или быть добавлен позже как внутренний оптимизационный слой **без влияния на контракт query**
  (т.е. не через глобальные mutable кэши и без зависимости от порядка событий).
