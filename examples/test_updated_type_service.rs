...
async fn main() -> Result<()> {
    ...
    match service.initialize().await {
        Ok(_) => {
            ...
            println!("✅ TypeSystemService v2.0 инициализирован: {:?}", final_state.current_stage);
    ...
    for query in search_queries {
        ...
        if let Some(first_result) = search_results.first() {
            println!("    Первый: {} - {}", first_result.name, first_result.description);
        }
    }
    ...
    println!("  - Платформенные типы: {}", unified_stats.platform_types_count);
    println!("  - Конфигурационные типы: {}", unified_stats.configuration_types_count);
    println!("  - Всего TypeResolution: {}", unified_stats.total_resolutions);
    println!("  - Запросы резолюции: {}", unified_stats.resolution_requests);
    
    if unified_stats.cache_hits + unified_stats.cache_misses > 0 {
        let hit_ratio = unified_stats.cache_hits as f64 / (unified_stats.cache_hits + unified_stats.cache_misses) as f64;
