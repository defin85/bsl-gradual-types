...
use crate::core::types::{TypeResolution, Certainty, ResolutionResult, ConcreteType, FacetKind};
...
pub trait TypeResolver: Send + Sync {
    ...
    async fn resolve(&self, expression: &str, context: &TypeContext, repository: &dyn TypeRepository) -> Result<TypeResolution>;
    
    /// Получить автодополнение для префикса
    async fn get_completions(&self, prefix: &str, context: &TypeContext, repository: &dyn TypeRepository) -> Result<Vec<CompletionItem>>;
}
...
impl TypeResolutionService {
    ...
    pub fn new(repository: Arc<dyn TypeRepository>) -> Self {
        let mut resolvers: Vec<Box<dyn TypeResolver>> = Vec::new();
    ...
    pub async fn resolve_expression(&self, expression: &str, context: &TypeContext) -> TypeResolution {
        ...
        for resolver in &self.resolvers {
            if resolver.can_resolve(expression) {
                match resolver.resolve(expression, context, self.repository.as_ref()).await {
                    Ok(resolution) => {
                    ...
                    Err(e) => {
                        println!("⚠️ Resolver failed for '{}': {}", expression, e);
                        continue;
                    }
                }
            }
    ...
    /// Получить автодополнение для префикса
    pub async fn get_completions(&self, prefix: &str, context: &TypeContext) -> Vec<CompletionItem> {
        let mut all_completions = Vec::new();
        
        // Собираем автодополнение от всех резолверов
        for resolver in &self.resolvers {
            if let Ok(completions) = resolver.get_completions(prefix, context, self.repository.as_ref()).await {
    ...
    async fn cache_resolution(&self, key: &str, resolution: &TypeResolution) {
        ...
        cache.insert(key.to_string(), CachedTypeResolution {
    ...
    async fn record_resolution_time(&self, duration: std::time::Duration) {
        ...
        if metrics.total_resolutions > 0 {
            metrics.average_resolution_time_ms = 
                (metrics.average_resolution_time_ms * (metrics.total_resolutions - 1) as f64 + time_ms) / 
...
impl PlatformTypeResolver {
    ...
    pub async fn initialize_cache(&self, repository: &dyn TypeRepository) -> Result<()> {
        ...
        let platform_types = repository.load_types_filtered(&super::data::TypeFilter {
            source: Some(TypeSource::Platform { version: "8.3".to_string() }),
            ..Default::default()
        }).await?;
        ...
        Ok(TypeResolution {
            ...
            result: ResolutionResult::Concrete(ConcreteType::Platform(
                crate::core::types::PlatformType {
                    ...
                    methods: raw_type.methods.iter().map(|m| crate::core::types::Method {
                        name: m.name.clone(),
                        parameters: m.parameters.iter().map(|p| crate::core::types::Parameter {
                            name: p.name.clone(),
                    ...
                    properties: raw_type.properties.iter().map(|p| crate::core::types::Property {
                        name: p.name.clone(),
                        type_: p.type_name.clone(),
            ...
            active_facet: None,
            available_facets: raw_type.available_facets.clone(),
        })
    }
}
...
impl TypeResolver for PlatformTypeResolver {
    fn can_resolve(&self, expression: &str) -> bool {
        ...
        let platform_patterns = [
            "Массив", "Array", "ТаблицаЗначений", "ValueTable", 
            "Структура", "Structure", "Соответствие", "Map",
            "СписокЗначений", "ValueList", "ДеревоЗначений", "ValueTree"
        ];
        
        platform_patterns.iter().any(|pattern| expression.contains(pattern))
    }
    
    async fn resolve(&self, expression: &str, _context: &TypeContext, _repository: &dyn TypeRepository) -> Result<TypeResolution> {
    ...
    async fn get_completions(&self, prefix: &str, _context: &TypeContext, _repository: &dyn TypeRepository) -> Result<Vec<CompletionItem>> {
        let cache = self.platform_types_cache.read().await;
...
impl TypeResolver for ConfigurationTypeResolver {
    ...
    async fn get_completions(&self, _prefix: &str, _context: &TypeContext, _repository: &dyn TypeRepository) -> Result<Vec<CompletionItem>> {
...
impl BslCodeResolver {
    ...
    pub async fn initialize_parser(&self) -> Result<()> {
        println!("🔧 Инициализация BslCodeResolver с tree-sitter...");
        
        match TreeSitterAdapter::new() {
            Ok(adapter) => {
                *self.parser.write().await = Some(adapter);
                println!("✅ BslCodeResolver готов с tree-sitter-bsl");
                Ok(())
            }
            Err(e) => {
                println!("⚠️ Tree-sitter недоступен: {}", e);
                Err(e)
            }
        }
    }
...
impl TypeResolver for BuiltinTypeResolver {
    fn can_resolve(&self, expression: &str) -> bool {
        let builtins = ["Строка", "String", "Число", "Number", "Булево", "Boolean", "Дата", "Date"];
...
impl TypeResolver for ExpressionResolver {
    fn can_resolve(&self, expression: &str) -> bool {
        // Сложные выражения с точками и скобками
        expression.contains('.') && (expression.contains('(') || expression.contains('['))
    }
    
    async fn resolve(&self, _expression: &str, _context: &TypeContext, _repository: &dyn TypeRepository) -> Result<TypeResolution> {
        // TODO: Реализовать анализ сложных выражений
        Ok(TypeResolution {
            certainty: Certainty::Unknown, // Честно признаём что пока не умеем
            result: ResolutionResult::Dynamic,
            source: crate::core::types::ResolutionSource::Inferred,
            metadata: crate::core::types::ResolutionMetadata::default(),
            active_facet: None,
    ...
    async fn get_completions(&self, _prefix: &str, _context: &TypeContext, _repository: &dyn TypeRepository) -> Result<Vec<CompletionItem>> {
        // TODO: Реализовать автодополнение для сложных выражений
        Ok(Vec::new())
    }
}
...
mod tests {
    ...
    async fn test_type_resolution_service() {
        ...
        if let Some(platform_resolver) = service.resolvers.iter()
            .find_map(|r| r.as_any().downcast_ref::<PlatformTypeResolver>()) {
            platform_resolver.initialize_cache(&**service.repository).await.unwrap();
        }
