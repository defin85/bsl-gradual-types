...
impl DocumentationCache {
        ...
        match self.config.eviction_strategy {
            ...
            EvictionStrategy::LFU => {
                ...
                if let Some((least_used_key, _)) =
                    cache.iter().min_by_key(|(_, entry)| entry.access_count)
                {
            ...
            EvictionStrategy::FIFO => {
                ...
                if let Some((oldest_key, _)) =
                    cache.iter().min_by_key(|(_, entry)| entry.created_at)
                {
                    let oldest_key = oldest_key.clone();
                    cache.remove(&oldest_key);
                }
    ...
    async fn update_cache_size_stats(&self) {
        ...
        stats.estimated_memory_mb =
            (type_cache_size * 1024 + search_cache_size * 512) as f64 / (1024.0 * 1024.0);
    }
}
