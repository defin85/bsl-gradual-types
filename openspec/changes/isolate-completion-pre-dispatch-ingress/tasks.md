...
- [ ] 1.1 Поднять authoritative completion timeline до response version `19` и contiguous baseline `contracts/lsp-completion-timeline/v16`, добавив bounded поля для раннего adapter ingress и wait окна `adapter read -> dispatch` без переосмысления legacy `transport_received_at_ms`.
- [ ] 1.2 Обновить derived extension verdicts и drilldown так, чтобы server pre-dispatch backlog давал отдельный verdict `adapter_before_dispatch_dominant`, а `client_before_transport_dominant` публиковался только для доказанного client-side wait.
