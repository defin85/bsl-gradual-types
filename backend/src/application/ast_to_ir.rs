            ...
            Expression::Call { function, .. } => {
                ...
                match function.as_ref() {
                    ...
                    Expression::PropertyAccess { object, property, .. } => {
                        ...
                        // MILESTONE 3.11: Graceful degradation для каскадных цепочек
                        // Если метод не найден, но тип известен (фасетный) — сохраняем его
                        // Это позволяет следующему звену цепочки продолжить резолвинг
                        if object_type.contains('.') {
                            // Фасетный тип (СправочникСсылка.Контрагенты) — сохраняем для цепочки
                            object_type.clone()
                        } else {
                            "Dynamic".to_string()
                        }
