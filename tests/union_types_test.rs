...
use bsl_gradual_types::core::types::{
    ...
    PrimitiveType, ResolutionSource, ResolutionMetadata
};
...
fn test_create_union_from_multiple_types() {
    ...
    match union.result {
        ResolutionResult::Union(union_types) => {
            ...
            let type_names: Vec<_> = union_types.iter()
                .map(|wt| &wt.type_)
                .collect();
...
fn test_union_compatibility() {
    ...
    assert!(UnionTypeManager::is_compatible_with_union(&create_string_type(), &union_types));
    
    // Number совместим с union
    assert!(UnionTypeManager::is_compatible_with_union(&create_number_type(), &union_types));
    
    // Boolean не совместим с union
    assert!(!UnionTypeManager::is_compatible_with_union(&create_boolean_type(), &union_types));
}
...
fn test_add_type_to_union() {
    let original_union = UnionTypeManager::create_union(vec![
        create_string_type(),
        create_number_type(),
    ]);
    
    let extended_union = UnionTypeManager::add_type_to_union(&original_union, create_boolean_type());
    ...
    match extended_union.result {
        ...
        _ => panic!("Expected Union type after adding, got: {:?}", extended_union.result),
    }
}
...
fn test_contains_type() {
    ...
    assert!(UnionTypeManager::contains_type(&union_types, &ConcreteType::Primitive(PrimitiveType::String)));
    assert!(UnionTypeManager::contains_type(&union_types, &ConcreteType::Primitive(PrimitiveType::Number)));
    assert!(!UnionTypeManager::contains_type(&union_types, &ConcreteType::Primitive(PrimitiveType::Boolean)));
}
...
fn test_get_type_weight() {
    ...
    let string_weight = UnionTypeManager::get_type_weight(&union_types, &ConcreteType::Primitive(PrimitiveType::String));
    assert!((string_weight - 0.3).abs() < 0.001);
    
    let number_weight = UnionTypeManager::get_type_weight(&union_types, &ConcreteType::Primitive(PrimitiveType::Number));
    assert!((number_weight - 0.7).abs() < 0.001);
    
    let boolean_weight = UnionTypeManager::get_type_weight(&union_types, &ConcreteType::Primitive(PrimitiveType::Boolean));
...
fn test_filter_union() {
    ...
    assert_eq!(filtered[0].type_, ConcreteType::Primitive(PrimitiveType::String));
}
