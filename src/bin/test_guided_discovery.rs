    ...
    if args.stats {
        ...
        for type_resolution in &config_types {
            ...
            if let ResolutionResult::Concrete(ConcreteType::Configuration(config)) = &type_resolution.result {
                ...
                if args.verbose {
                    println!("  - {:?}: {} (атрибутов: {})", 
                        config.kind, config.name, config.attributes.len());
                }
            }
        }
...
fn test_with_integration(args: &Args) -> Result<()> {
    ...
    println!("✅ PlatformTypeResolver с guided discovery создан за {:?}", elapsed);
    println!("📊 Platform globals: {}", resolver.get_platform_globals_count());
