...
КонецПроцедуры\n";
        let uri =
            Url::parse("file:///test_p7_interactive_type_index_reasons.bsl").expect("test uri");
        let did_open = DidOpenTextDocumentParams {
            text_document: TextDocumentItem {
                uri: uri.clone(),
                language_id: "bsl".to_string(),
        ...
        let completion_response = service
            ...
            .await
            .expect("completion request");
        assert!(completion_response.is_some(), "completion must return a response");
        ...
        let hover_response = service
            ...
            .await
            .expect("hover request");
        assert!(hover_response.is_some(), "hover must return a response envelope");
