...
#[cfg(debug_assertions)]
fn debug_did_save_followup_phase_v2(
    key: &super::super::DiagnosticsSupersessionKeyV2,
    pipeline_started: Instant,
    phase: &str,
) {
    if std::env::var_os("BSL_DEBUG_DID_SAVE_FOLLOWUP_PHASES").is_none()
        || !matches!(
            key.profile,
            bsl_runtime::application::DiagnosticsProfile::IdleHeavy
        )
        || key.save_cycle_sequence.is_none()
    {
        return;
    }
    eprintln!(
        "bsl_debug_did_save_followup_phase file_id={} generation={} sequence={} version={} elapsed_ms={} phase={}",
        key.file_id.0,
        key.diagnostics_generation,
        key.save_cycle_sequence.unwrap_or_default(),
...
#[cfg(not(debug_assertions))]
fn debug_did_save_followup_phase_v2(
    _key: &super::super::DiagnosticsSupersessionKeyV2,
    _pipeline_started: Instant,
    _phase: &str,
) {
}

#[cfg(debug_assertions)]
fn debug_did_save_followup_branch_context_phase_v2(
    key: &super::super::DiagnosticsSupersessionKeyV2,
    branch_context_started: Instant,
    phase: &str,
) {
    if std::env::var_os("BSL_DEBUG_DID_SAVE_FOLLOWUP_PHASES").is_none()
        || !matches!(
    ...
    eprintln!(
        "bsl_debug_did_save_followup_branch_context file_id={} generation={} sequence={} version={} branch_elapsed_ms={} phase={}",
        key.file_id.0,
        key.diagnostics_generation,
        key.save_cycle_sequence.unwrap_or_default(),
        key.requested_version,
        branch_context_started.elapsed().as_millis(),
        phase
    );
}
...
fn debug_did_save_followup_branch_context_phase_v2(
    _key: &super::super::DiagnosticsSupersessionKeyV2,
    _branch_context_started: Instant,
    _phase: &str,
) {
}
...
fn is_started_parser_base_recovery_same_family_contour_v2(
    ...
    branch_context.ready_snapshot_task_state == ReadySnapshotTaskStateV2::InFlightSameVersion
        && matches!(
            ...
            Some(super::super::DidSaveExactProducerLifecycleStateV2::Started)
        )
        && phase_attribution.is_some_and(|attribution| {
...
pub(crate) struct DiagnosticsReadySnapshotPhaseAttributionV2 {
    ...
    pub(crate) timeout_leaf: Option<&'static str>,
    pub(crate) timeout_leaf_elapsed_ms: Option<u64>,
    ...
    pub(crate) program_lowering_fully_reused_top_level_node_count: Option<u64>,
    pub(crate) program_lowering_fully_rebuilt_top_level_node_count: Option<u64>,
    pub(crate) program_lowering_routine_body_reuse_node_count: Option<u64>,
    pub(crate) program_lowering_fully_reused_top_level_lowering_units: Option<u64>,
    pub(crate) program_lowering_fully_rebuilt_top_level_lowering_units: Option<u64>,
...
impl DiagnosticsReadySnapshotPhaseAttributionV2 {
    ...
    pub(crate) fn from_snapshot(
        ...
        let (timeout_leaf, timeout_leaf_elapsed_ms) = if include_timeout_phase {
            match snapshot.current_phase {
                Some(super::super::ReadyParseSnapshotAttributionPhaseV2::Waiting) => {
                    ("waiting", snapshot.current_phase_elapsed_ms)
                }
                Some(super::super::ReadyParseSnapshotAttributionPhaseV2::ParseExec) => {
                    match snapshot.current_parse_exec_subphase {
                        None => (
                            "before_first_parse_exec_subphase",
                            snapshot.current_phase_elapsed_ms,
                        ),
                        Some(
                            super::super::ReadyParseSnapshotParseExecSubphaseV2::CoreParseBuild,
                        ) => match snapshot.current_core_build_checkpoint {
                            None => (
                        ...
                        Some(subphase) => {
                            (subphase.as_str(), snapshot.current_parse_exec_subphase_elapsed_ms)
                        }
                    }
                }
                Some(super::super::ReadyParseSnapshotAttributionPhaseV2::PostParsePreMaterialization) => (
                    "post_parse_pre_materialization",
                    snapshot.current_phase_elapsed_ms,
                ),
                Some(super::super::ReadyParseSnapshotAttributionPhaseV2::ReadyInstall) => {
                    ("ready_install", snapshot.current_phase_elapsed_ms)
                }
                Some(super::super::ReadyParseSnapshotAttributionPhaseV2::DocumentSymbolSideWork) => (
        ...
        let attribution = Self {
            ...
            timeout_phase_elapsed_ms: include_timeout_phase
                ...
                .flatten(),
            timeout_leaf: (include_timeout_phase && !timeout_leaf.is_empty()).then_some(timeout_leaf),
            timeout_leaf_elapsed_ms: (include_timeout_phase && !timeout_leaf.is_empty())
                .then_some(timeout_leaf_elapsed_ms)
                .flatten(),
...
struct SemanticDiagnosticsIrBuildBreakdownV2 {
    ast_to_ir_convert_ms: Option<Duration>,
    semantic_facts_materialize_ms: Option<Duration>,
    semantic_facts_seed_module_context_ms: Option<Duration>,
    semantic_facts_local_function_summaries_ms: Option<Duration>,
    semantic_facts_local_function_summaries_prep_ms: Option<Duration>,
    semantic_facts_local_function_summaries_fixed_point_ms: Option<Duration>,
    semantic_facts_local_function_summaries_snapshot_build_ms: Option<Duration>,
    semantic_facts_local_function_summaries_body_infer_ms: Option<Duration>,
...
impl SemanticDiagnosticsIrBuildBreakdownV2 {
    fn from_ir_build_profile(profile: bsl_analysis_v2::IrBuildProfile) -> Self {
        ...
        Self {
            ...
            semantic_facts_materialize_ms: (profile.semantic_facts_materialize_ms > 0)
                .then(|| duration_from_profile_ms(profile.semantic_facts_materialize_ms)),
            semantic_facts_seed_module_context_ms: (profile.semantic_facts_seed_module_context_ms
                > 0)
            .then(|| duration_from_profile_ms(profile.semantic_facts_seed_module_context_ms)),
            semantic_facts_local_function_summaries_ms: (profile
            ...
            semantic_facts_visit_callable_body_ms: (profile.semantic_facts_visit_callable_body_ms
                > 0)
            .then(|| duration_from_profile_ms(profile.semantic_facts_visit_callable_body_ms)),
            semantic_facts_merge_control_flow_env_ms: (profile
                .semantic_facts_merge_control_flow_env_ms
                > 0)
            .then(|| duration_from_profile_ms(profile.semantic_facts_merge_control_flow_env_ms)),
            semantic_facts_visit_callable_body_count: (profile
                .semantic_facts_visit_callable_body_count
                > 0)
            .then_some(profile.semantic_facts_visit_callable_body_count),
            ...
            semantic_facts_local_function_summary_count: (profile
                ...
                > 0)
            .then_some(profile.semantic_facts_local_function_summary_count),
            semantic_facts_index_entry_count: (profile.semantic_facts_index_entry_count > 0)
                .then_some(profile.semantic_facts_index_entry_count),
        }
    }

    fn has_any(self) -> bool {
        self.ast_to_ir_convert_ms.is_some()
            || self.semantic_facts_materialize_ms.is_some()
            || self.semantic_facts_seed_module_context_ms.is_some()
            || self.semantic_facts_local_function_summaries_ms.is_some()
            || self
...
impl SemanticDiagnosticsQueryBreakdownV2 {
    fn from_profiled_result(profiled: &bsl_analysis_v2::SemanticDiagnosticsProfiledResult) -> Self {
        let duration_from_profile_ms =
            |value: u128| Duration::from_millis(value.min(u64::MAX as u128) as u64);
        Self {
            inputs_ms: Some(duration_from_profile_ms(profiled.profile.inputs_ms)),
            parse_result_ms: Some(duration_from_profile_ms(profiled.profile.parse_result_ms)),
            ir_ms: Some(duration_from_profile_ms(profiled.profile.ir_ms)),
            collect_ms: Some(duration_from_profile_ms(profiled.profile.collect_ms)),
            flow_sensitive_ms: (profiled.profile.flow_sensitive_ms > 0)
    ...
    fn has_any(self) -> bool {
        self.inputs_ms.is_some()
            || self.parse_result_ms.is_some()
            || self.ir_ms.is_some()
...
impl BslLanguageServer {
    ...
    async fn finalize_diagnostics_save_profile_result_v2(
        ...
        semantic_diagnostics_query_breakdown: Option<SemanticDiagnosticsQueryBreakdownV2>,
        ...
        let publish = (matches!(
            ...
            || semantic_diagnostics_query_breakdown.is_some_and(|value| value.has_any())
        ...
        .then(|| crate::types::DiagnosticsSaveTimelinePublishTrace {
            ...
            semantic_diagnostics_query_ms: semantic_diagnostics_query_ms
                .map(|value| value.as_millis().min(u64::MAX as u128) as u64),
            semantic_diagnostics_inputs_ms: semantic_diagnostics_query_breakdown
                .and_then(|value| duration_to_nonzero_ms(value.inputs_ms)),
            semantic_diagnostics_parse_result_ms: semantic_diagnostics_query_breakdown
                .and_then(|value| duration_to_nonzero_ms(value.parse_result_ms)),
            semantic_diagnostics_ir_ms: semantic_diagnostics_query_breakdown
                .and_then(|value| duration_to_nonzero_ms(value.ir_ms)),
            semantic_diagnostics_collect_ms: semantic_diagnostics_query_breakdown
                .and_then(|value| duration_to_nonzero_ms(value.collect_ms)),
            semantic_diagnostics_flow_sensitive_ms: semantic_diagnostics_query_breakdown
                .and_then(|value| duration_to_nonzero_ms(value.flow_sensitive_ms)),
            semantic_diagnostics_ir_ast_to_ir_convert_ms: semantic_diagnostics_query_breakdown
                .and_then(|value| value.ir_build_breakdown)
                .and_then(|value| duration_to_nonzero_ms(value.ast_to_ir_convert_ms)),
            semantic_diagnostics_ir_semantic_facts_materialize_ms:
                semantic_diagnostics_query_breakdown
                    .and_then(|value| value.ir_build_breakdown)
                    .and_then(|value| duration_to_nonzero_ms(value.semantic_facts_materialize_ms)),
            semantic_diagnostics_ir_semantic_facts_seed_module_context_ms:
                semantic_diagnostics_query_breakdown
                    .and_then(|value| value.ir_build_breakdown)
                    .and_then(|value| {
                        duration_to_nonzero_ms(value.semantic_facts_seed_module_context_ms)
                    }),
            semantic_diagnostics_ir_semantic_facts_local_function_summaries_ms:
                semantic_diagnostics_query_breakdown
                    .and_then(|value| value.ir_build_breakdown)
                    .and_then(|value| {
                        duration_to_nonzero_ms(value.semantic_facts_local_function_summaries_ms)
                    }),
            ...
            semantic_diagnostics_ir_semantic_facts_local_function_summaries_function_count:
                semantic_diagnostics_query_breakdown
                    .and_then(|value| value.ir_build_breakdown)
                    .and_then(|value| {
                        value.semantic_facts_local_function_summaries_function_count
                    }),
            semantic_diagnostics_ir_semantic_facts_local_function_summaries_scc_count:
                semantic_diagnostics_query_breakdown
                    .and_then(|value| value.ir_build_breakdown)
                    .and_then(|value| value.semantic_facts_local_function_summaries_scc_count),
            ...
            semantic_diagnostics_ir_semantic_facts_visit_statements_ms:
                semantic_diagnostics_query_breakdown
                    .and_then(|value| value.ir_build_breakdown)
                    .and_then(|value| {
                        duration_to_nonzero_ms(value.semantic_facts_visit_statements_ms)
                    }),
            semantic_diagnostics_ir_semantic_facts_visit_callable_body_ms:
                semantic_diagnostics_query_breakdown
                    .and_then(|value| value.ir_build_breakdown)
                    .and_then(|value| {
                        duration_to_nonzero_ms(value.semantic_facts_visit_callable_body_ms)
                    }),
            semantic_diagnostics_ir_semantic_facts_merge_control_flow_env_ms:
                semantic_diagnostics_query_breakdown
                    .and_then(|value| value.ir_build_breakdown)
                    .and_then(|value| {
                        duration_to_nonzero_ms(value.semantic_facts_merge_control_flow_env_ms)
                    }),
            semantic_diagnostics_ir_semantic_facts_visit_callable_body_count:
                semantic_diagnostics_query_breakdown
                    .and_then(|value| value.ir_build_breakdown)
                    .and_then(|value| value.semantic_facts_visit_callable_body_count),
            ...
            semantic_diagnostics_ir_semantic_facts_statement_count:
                semantic_diagnostics_query_breakdown
                    .and_then(|value| value.ir_build_breakdown)
                    .and_then(|value| value.semantic_facts_statement_count),
            semantic_diagnostics_ir_semantic_facts_local_function_summary_count:
                semantic_diagnostics_query_breakdown
                    .and_then(|value| value.ir_build_breakdown)
                    .and_then(|value| value.semantic_facts_local_function_summary_count),
    ...
    async fn diagnostics_save_followup_branch_context_v2(
        &self,
        supersession_key: &super::super::DiagnosticsSupersessionKeyV2,
        include_ready_state: bool,
    ) -> DiagnosticsSaveFollowupBranchContextV2 {
        let branch_context_started = Instant::now();
        debug_did_save_followup_branch_context_phase_v2(
            supersession_key,
            branch_context_started,
            "start",
        );
        ...
        debug_did_save_followup_branch_context_phase_v2(
            supersession_key,
            branch_context_started,
            "after_detached_artifact_read",
        );
        if save_family_detached_artifact_available
            && matches!(
        ...
        {
            return DiagnosticsSaveFollowupBranchContextV2 {
                ready_snapshot_task_state: ReadySnapshotTaskStateV2::InFlightSameVersion,
                producer_lifecycle_state,
                shadow_state_available: shadow_state.is_some(),
                shadow_text_hash,
                ready_snapshot_phase_attribution: None,
            };
        }
        let ready_state = if include_ready_state {
            self.ready_parse_snapshot_state_for_target_v2(
                supersession_key.file_id,
                supersession_key.requested_version,
                shadow_text_hash,
            )
    ...
    fn record_diagnostics_save_followup_profile_phase_v2(
        &self,
        uri: &Url,
        supersession_key: &super::super::DiagnosticsSupersessionKeyV2,
        phase: &'static str,
        ...
        self.record_diagnostics_save_timeline_followup_profile_phase(
            uri,
            cycle_key,
            phase,
            pipeline_started.elapsed(),
    ...
    fn record_diagnostics_save_followup_probe_state_v2(
        ...
        if let Some(producer_lifecycle_state) = producer_lifecycle_state {
            ...
            if ready_snapshot_wait_probe == Some(ReadyParseSnapshotProbeOutcomeV2::Timeout) {
                self.record_diagnostics_save_timeline_followup_producer_lifecycle_state_at_timeout(
                    uri,
                    cycle_key,
                    producer_lifecycle_state.as_str(),
                );
    ...
    fn record_diagnostics_save_followup_final_producer_lifecycle_state_v2(
        &self,
        uri: &Url,
        supersession_key: &super::super::DiagnosticsSupersessionKeyV2,
        producer_lifecycle_state: super::super::DidSaveExactProducerLifecycleStateV2,
    ) {
        let Some(cycle_key) =
            Self::diagnostics_save_cycle_key_from_supersession_key_v2(supersession_key)
        else {
        ...
        self.record_diagnostics_save_timeline_followup_producer_final_lifecycle_state(
            uri,
            cycle_key,
            producer_lifecycle_state.as_str(),
        );
    ...
    async fn try_execute_save_followup_from_shadow_state_v2(
        ...
        let branch_context = self
            .diagnostics_save_followup_branch_context_v2(supersession_key, false)
            .await;
    ...
    async fn execute_save_followup_from_parse_snapshot_artifact_v2(
        ...
        debug_did_save_followup_phase_v2(
            supersession_key,
            pipeline_started,
            "artifact_execute_start",
        );
        self.record_diagnostics_save_followup_profile_phase_v2(
            uri,
            supersession_key,
            "artifact_execute_start",
            pipeline_started,
        ...
        debug_did_save_followup_phase_v2(
            supersession_key,
            pipeline_started,
            "artifact_after_execution_context",
        );
        let support_bundle = self.analysis_v2.completion_support_bundle();
        debug_did_save_followup_phase_v2(
            supersession_key,
            pipeline_started,
            "artifact_after_support_bundle",
        );
        let path = match uri.to_file_path() {
            Ok(path) => path.to_string_lossy().to_string(),
            Err(_) => uri.to_string(),
        ...
        debug_did_save_followup_phase_v2(
            supersession_key,
            pipeline_started,
            "artifact_after_syntax_artifact",
        );
        let deps_id = support_bundle.deps_id.clone();
        let deps = support_bundle.deps.clone();
        let settings_id = context.settings.settings_id.clone();
        ...
        let debug_supersession_key = *supersession_key;
        let debug_pipeline_started = pipeline_started;
        let run_followup = move || {
            debug_did_save_followup_phase_v2(
                &debug_supersession_key,
                debug_pipeline_started,
                "artifact_blocking_start",
            );
            let setup_started = Instant::now();
            let mut host = bsl_analysis_v2::AnalysisHostV2::default();
            host.apply_change(bsl_analysis_v2::Change::SetDepsSnapshot {
            ...
            debug_did_save_followup_phase_v2(
                &debug_supersession_key,
                debug_pipeline_started,
                "artifact_before_semantic_query",
            );
            let mut semantic_parse_source = None;
            let mut semantic_ir_source = None;
            let mut semantic_materialization_path = None;
        ...
        debug_did_save_followup_phase_v2(
            supersession_key,
            pipeline_started,
            "artifact_before_blocking_dispatch",
        );
        self.record_diagnostics_save_followup_profile_phase_v2(
            uri,
            supersession_key,
        ...
        debug_did_save_followup_phase_v2(
            supersession_key,
            pipeline_started,
            "artifact_after_blocking_dispatch",
        );
        self.record_diagnostics_save_followup_profile_phase_v2(
            uri,
            supersession_key,
        ...
        debug_did_save_followup_phase_v2(
            supersession_key,
            pipeline_started,
            "artifact_before_publish_checkpoint",
        );
        self.record_diagnostics_save_followup_profile_phase_v2(
            uri,
            supersession_key,
        ...
        debug_did_save_followup_phase_v2(
            supersession_key,
            pipeline_started,
            "artifact_after_publish_checkpoint",
        );
        self.record_diagnostics_save_followup_profile_phase_v2(
            uri,
            supersession_key,
    ...
    pub(crate) async fn try_execute_save_followup_from_ready_artifacts_v2(
        ...
        debug_did_save_followup_phase_v2(
            supersession_key,
            pipeline_started,
            "ready_artifacts_after_expected_hash",
        );
        let probe = self
        ...
        debug_did_save_followup_phase_v2(
            supersession_key,
            pipeline_started,
            "ready_artifacts_after_branch_context",
        );
        ...
        if let Some(ready_state) = probe.state {
            debug_did_save_followup_phase_v2(
                supersession_key,
                pipeline_started,
                "ready_artifacts_before_ready_state_execute",
            );
        ...
        if let Some(detached_artifact) = probe.detached_artifact {
            debug_did_save_followup_phase_v2(
                supersession_key,
                pipeline_started,
                "ready_artifacts_before_detached_execute",
            );
            return SaveFollowupReadyArtifactsAttemptV2::Executed(
                self.execute_save_followup_from_parse_snapshot_artifact_v2(
                    uri,
                    ...
                    ReadyParseSnapshotProbeOutcomeV2::Timeout => {
                            ...
                            .await
                        {
                            ...
                            ReadySnapshotContinuationResultV2::FallbackExhaustedProof => {
                                if is_started_parser_base_recovery_same_family_contour_v2(
                                    branch_context,
                                    phase_attribution,
                                ) {
                                    self.record_diagnostics_save_followup_final_producer_lifecycle_state_v2(
                                        uri,
                                        supersession_key,
                                        super::super::DidSaveExactProducerLifecycleStateV2::ContinuityLost,
                                    );
                                }
                                self.record_diagnostics_save_followup_continuation_reason_v2(
                                    uri,
    ...
    async fn execute_diagnostics_profile_once_v2(
        ...
        debug_did_save_followup_phase_v2(&supersession_key, pipeline_started, "profile_start");
        if let Some(disposition) = self
            .diagnostics_checkpoint_v2(&supersession_key, trigger, cancel_token)
            .await
        {
        ...
        debug_did_save_followup_phase_v2(&supersession_key, pipeline_started, "after_settings");
        let plan =
        ...
        if save_followup_from_did_save && run_semantic {
            debug_did_save_followup_phase_v2(
                &supersession_key,
                pipeline_started,
                "followup_before_pending_publish",
            );
            self.record_diagnostics_save_followup_profile_phase_v2(
                uri,
                &supersession_key,
            ...
            debug_did_save_followup_phase_v2(
                &supersession_key,
                pipeline_started,
                "followup_after_save_fastlane_gate",
            );
            self.record_diagnostics_save_followup_profile_phase_v2(
                uri,
                &supersession_key,
                ...
                .await
            {
                DidSaveFollowupAdmissionOutcome::Admitted {
                    ...
                    debug_did_save_followup_phase_v2(
                        &supersession_key,
                        pipeline_started,
                        "followup_after_admission_admitted",
                    );
                    self.record_diagnostics_save_followup_profile_phase_v2(
                        uri,
                        &supersession_key,
                ...
                DidSaveFollowupAdmissionOutcome::Disposition {
                    ...
                    debug_did_save_followup_phase_v2(
                        &supersession_key,
                        pipeline_started,
                        "followup_after_admission_disposition",
                    );
                    self.record_diagnostics_pipeline_event_v2(
                        trigger,
                        supersession_key.profile,
            ...
            debug_did_save_followup_phase_v2(
                &supersession_key,
                pipeline_started,
                "followup_after_fastlane_artifact_check",
            );
            self.record_diagnostics_save_followup_profile_phase_v2(
                uri,
                &supersession_key,
            ...
            if save_fastlane_gate_open {
                debug_did_save_followup_phase_v2(
                    &supersession_key,
                    pipeline_started,
                    "followup_before_branch_context",
                );
                self.record_diagnostics_save_followup_profile_phase_v2(
                    uri,
                    &supersession_key,
                ...
                if let SaveFollowupReadyArtifactsAttemptV2::Executed(disposition) =
                    zero_probe_attempt
                {
                    return disposition;
                }
                if prefer_inflight_exact_wait {
                    debug_did_save_followup_phase_v2(
                        &supersession_key,
                        pipeline_started,
                        "followup_before_promote_exact_producer",
                    );
                    let _ = self
                    ...
                    debug_did_save_followup_phase_v2(
                        &supersession_key,
                        pipeline_started,
                        "followup_after_promote_exact_producer",
                    );
                    debug_did_save_followup_phase_v2(
                        &supersession_key,
                        pipeline_started,
                        "followup_before_bounded_wait_probe",
                    );
                    self.record_diagnostics_save_followup_profile_phase_v2(
                        uri,
                        &supersession_key,
                    ...
                    debug_did_save_followup_phase_v2(
                        &supersession_key,
                        pipeline_started,
                        "followup_after_bounded_wait_probe",
                    );
                    self.record_diagnostics_save_followup_profile_phase_v2(
                        uri,
                        &supersession_key,
                        ...
                        .await
                    {
                        debug_did_save_followup_phase_v2(
                            &supersession_key,
                            pipeline_started,
                            "followup_after_relief_valve_executed",
                        );
                        return disposition;
                    }
                    debug_did_save_followup_phase_v2(
                        &supersession_key,
                        pipeline_started,
                        "followup_after_relief_valve_none",
                    );
                }
                debug_did_save_followup_phase_v2(
                    &supersession_key,
                    pipeline_started,
                    "followup_before_shadow_state",
                );
                if let Some(disposition) = self
                    ...
                    .await
                {
                    debug_did_save_followup_phase_v2(
                        &supersession_key,
                        pipeline_started,
                        "followup_after_shadow_state_executed",
                    );
                    return disposition;
                }
                debug_did_save_followup_phase_v2(
                    &supersession_key,
                    pipeline_started,
                    "followup_after_shadow_state_none",
                );
            }
        ...
        debug_did_save_followup_phase_v2(
            &supersession_key,
            pipeline_started,
            "generic_before_execution_context",
        );
        let context = self
            .build_execution_context_v2(
                bsl_runtime::application::SemanticOperation::Diagnostics,
        ...
        debug_did_save_followup_phase_v2(
            &supersession_key,
            pipeline_started,
            "generic_after_execution_context",
        );
        debug_did_save_followup_phase_v2(
            &supersession_key,
            pipeline_started,
            "generic_before_prepare_stateful",
        );
        ...
        debug_did_save_followup_phase_v2(
            &supersession_key,
            pipeline_started,
            "generic_after_prepare_stateful",
        );
        ...
        if was_cancelled {
            ...
            return self
                .finalize_diagnostics_save_profile_result_v2(
                    ...
                    followup_semantic_query_breakdown,
                    None,
                    None,
                    followup_semantic_path,
        ...
        if run_semantic {
            ...
            let semantic_call = bsl_runtime::application::spawn_bounded_blocking_with_class_observed_call_origin_lane_hooks(
                ...
                move || {
                    ...
                    match query {
                        Ok(Some(profiled)) => {
                            let semantic_query_breakdown =
                                SemanticDiagnosticsQueryBreakdownV2::from_profiled_result(
                                    &profiled,
                                );
                            coordinator_for_blocking
                                .record_intellisense_v2_semantic_diagnostics_query_breakdown(
