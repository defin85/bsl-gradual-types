...
"#;
    ...
    assert!(
        hover1.contains("🟢 Known")
            || hover1.contains("🟡 Inferred")
            || hover1.contains("⚪ Unknown"),
        "Hover должен содержать certainty индикатор\nActual: {}",
        hover1
    );
    assert!(
        hover1.contains("Уверенность:"),
        "Hover должен содержать метку 'Уверенность:'\nActual: {}",
        hover1
    );
    ...
    assert!(
        hover2.contains("🟢 Known")
            || hover2.contains("🟡 Inferred")
            || hover2.contains("⚪ Unknown"),
        "Hover должен содержать certainty индикатор\nActual: {}",
        hover2
    );
    assert!(
...
"#;
    ...
    match (certainty_pos, warning_pos) {
        ...
        (None, _) => {
            panic!(
                "Certainty ОТСУТСТВУЕТ в hover (БАГ НЕ ИСПРАВЛЕН!)\nActual: {}",
                hover
            );
        }
    }
...
"#;
    ...
    // 1. Имя переменной
    assert!(
        hover.contains("МойМассив") || hover.contains("Переменная:"),
    ...
    // 2. Тип
    assert!(
        hover.contains("Массив") || hover.contains("Тип:"),
