...
use bsl_gradual_types::documentation::{
    RenderEngine, DocumentationSearchEngine, AdvancedSearchQuery
};
use bsl_gradual_types::documentation::core::hierarchy::{TypeHierarchy, CategoryNode};
use std::fs;
...
async fn main() -> Result<()> {
    ...
    println!("✅ Тестовая иерархия создана: {} категорий", test_hierarchy.root_categories.len());
    ...
    println!("🔍 Результаты поиска: {} найдено", search_results.total_count);
    
    // Рендеринг результатов поиска
    let search_html = render_engine.html_renderer.render_search_results(&search_results).await?;
...
fn create_test_hierarchy() -> TypeHierarchy {
    use bsl_gradual_types::documentation::core::hierarchy::{
        RootCategoryNode, DocumentationNode, SubCategoryNode,
        HierarchyStatistics, NavigationIndex
    };
    ...
    let root_category = RootCategoryNode {
        ...
        children: vec![
            DocumentationNode::SubCategory(SubCategoryNode {
                ...
                hierarchy_path: vec!["Global Context".to_string(), "Universal collections".to_string()],
            }),
            DocumentationNode::SubCategory(SubCategoryNode {
