...
use tree_sitter::{Parser as TSParser, Node, Language};
use crate::parser::ast::{Program, Statement, Expression, Parameter, BinaryOp, UnaryOp};
...
impl TreeSitterAdapter {
    ...
    fn convert_statement(&self, node: Node) -> Result<Option<Statement>> {
        match node.kind() {
            "procedure_definition" => {
                Ok(Some(self.convert_procedure(node)?))
            }
            "function_definition" => {
                Ok(Some(self.convert_function(node)?))
            }
            "var_definition" | "var_statement" => {
                Ok(Some(self.convert_var_declaration(node)?))
            }
            "assignment_statement" => {
                Ok(Some(self.convert_assignment(node)?))
            }
            "if_statement" => {
                Ok(Some(self.convert_if_statement(node)?))
            }
            "while_statement" => {
                Ok(Some(self.convert_while_statement(node)?))
            }
            "for_statement" | "for_each_statement" => {
                Ok(Some(self.convert_for_statement(node)?))
            }
            "return_statement" => {
                Ok(Some(self.convert_return_statement(node)?))
            }
            "call_statement" => {
                Ok(Some(self.convert_call_statement(node)?))
            }
            "break_statement" => {
    ...
    fn convert_expression(&self, node: Node) -> Result<Expression> {
        match node.kind() {
            ...
            "identifier" => {
                Ok(Expression::Identifier(self.get_node_text(node)))
            }
            "number" => {
            ...
            "binary_expression" => {
                self.convert_binary_expression(node)
            }
            "unary_expression" => {
                self.convert_unary_expression(node)
            }
            "call_expression" | "method_call" => {
                self.convert_call_expression(node)
            }
            "property_access" => {
                self.convert_member_access(node)
            }
            "new_expression" => {
                self.convert_new_expression(node)
            }
            "ternary_expression" => {
                self.convert_ternary_expression(node)
            }
            "const_expression" => {
            ...
            _ => {
                ...
                if !matches!(node.kind(), "(" | ")" | ";" | "," | "=" | "THEN_KEYWORD" | "ELSE_KEYWORD") {
    ...
    fn convert_ternary_expression(&self, node: Node) -> Result<Expression> {
        ...
        if let (Some(condition), Some(then_expr), Some(else_expr)) = (condition, then_expr, else_expr) {
    ...
    fn is_statement_node(&self, node: Node) -> bool {
        matches!(
            node.kind(),
            "procedure_definition" | "function_definition" | "var_definition" | 
            "var_statement" | "assignment_statement" | "if_statement" |
            "while_statement" | "for_statement" | "for_each_statement" |
            "return_statement" | "call_statement" | "break_statement" |
    ...
    fn is_expression_node(&self, node: Node) -> bool {
        matches!(
            node.kind(),
            "identifier" | "number" | "string" | "boolean" | "date" |
            "UNDEFINED_KEYWORD" | "NULL_KEYWORD" | "binary_expression" |
            "unary_expression" | "call_expression" | "method_call" |
            "property_access" | "new_expression" | "ternary_expression" |
            "const_expression" | "expression" | "index_access" | 
