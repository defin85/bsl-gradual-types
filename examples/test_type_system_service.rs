...
async fn main() -> Result<()> {
    ...
    match service.initialize().await {
        Ok(_) => {
            ...
            println!("✅ Инициализация завершена: {:?}", final_state.current_stage);
            ...
            if let Some(start_time) = final_state.start_time {
                println!("⏱️ Время инициализации: {:.2}s", start_time.elapsed().as_secs_f64());
            }
        }
        Err(e) => {
    ...
    for expression in expressions {
        ...
        println!("💡 Автодополнение для '{}': {} вариантов", expression, completions.len());
    }
    ...
    for query in search_queries {
        ...
        match service.search(search_query).await {
            Ok(results) => {
                println!("🔍 Поиск '{}': {} результатов за {}ms", 
