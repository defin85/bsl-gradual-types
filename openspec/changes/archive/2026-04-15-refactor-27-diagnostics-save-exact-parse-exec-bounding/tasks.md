...
- [x] 1.1 Add a save-critical exact parse path for same-version `didSave` follow-up so the
...
- [x] 2.1 Split exact `parse_exec` into bounded observable subphases or checkpoints that can
      surface which exact in-parse slice dominates the timeout path.
- [x] 2.2 Export the new subphase attribution through diagnostics save timeline / incident bundle
...
- [x] 4.1 Run targeted backend tests covering save-critical exact `parse_exec`, timeout subphase
      attribution, and the relevant `didSave` follow-up path.
- [x] 4.2 Run `openspec validate refactor-27-diagnostics-save-exact-parse-exec-bounding --strict --no-interactive`.

## 5. OpenSpec / Beads Sync

- [x] 5.1 Keep Beads epic `bsl-gradual-types-d3t6` and children
      `bsl-gradual-types-d3t6.1` through `bsl-gradual-types-d3t6.4` aligned with the intended
