...
class VersionedContractsScriptTest(unittest.TestCase):
    ...
    def test_completion_timeline_latest_baseline_is_v19_and_response_version_22(self) -> None:
        module = self.load_script_module()
        contract = module.parse_json(self.LSP_TIMELINE_CONTRACT)

        self.assertEqual(module.REQUIRED_LATEST_MAJORS["lsp-completion-timeline"], 19)
        self.assertEqual(contract["major_version"], 19)
        self.assertEqual(contract["response"]["version"], 22)
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            module.REQUIRED_V19_TIMELINE_SERVER_EDGE_DETAILS_FIELDS,
        )
    ...
    def test_completion_timeline_v19_rejects_wrong_response_version(self) -> None:
        module = self.load_script_module()
        with tempfile.TemporaryDirectory(prefix="versioned-contracts-v19-version-") as tmp_dir:
            surface_dir = Path(tmp_dir) / "lsp-completion-timeline"
            shutil.copytree(
                self.REPO_ROOT / "contracts" / "lsp-completion-timeline",
                surface_dir,
            )
            contract_path = surface_dir / "v19" / "contract.json"
            payload = json.loads(contract_path.read_text(encoding="utf-8"))
            payload["response"]["version"] = 21
            ...
            self.assertIsNotNone(
                failure,
                "validate_surface_contract() must reject v19 contract with wrong response.version",
            )

    def test_completion_timeline_v19_rejects_non_exact_server_edge_field_sets(self) -> None:
        module = self.load_script_module()
        ...
        for mutation in ("rename", "extra", "missing"):
            with self.subTest(mutation=mutation):
                with tempfile.TemporaryDirectory(
                    prefix=f"versioned-contracts-v19-fields-{mutation}-"
                ) as tmp_dir:
                    surface_dir = Path(tmp_dir) / "lsp-completion-timeline"
                    shutil.copytree(
                        self.REPO_ROOT / "contracts" / "lsp-completion-timeline",
                    ...
                    self.assertIsNotNone(
                        failure,
                        "validate_surface_contract() must reject non-exact v19 server edge field sets",
                    )

    def test_completion_timeline_v19_rejects_non_exact_query_bundle_stage_names(self) -> None:
        module = self.load_script_module()

        with tempfile.TemporaryDirectory(prefix="versioned-contracts-v19-query-bundle-stage-names-") as tmp_dir:
            surface_dir = Path(tmp_dir) / "lsp-completion-timeline"
            shutil.copytree(
                self.REPO_ROOT / "contracts" / "lsp-completion-timeline",
                surface_dir,
            ...
            self.assertIsNotNone(
                failure,
                "validate_surface_contract() must reject non-exact v19 query_bundle_stage_names",
            )
