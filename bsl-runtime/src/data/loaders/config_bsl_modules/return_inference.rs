...
use bsl_shared::domain::ReturnDomain;
...
pub(crate) fn infer_return_types_across_modules(
...
) -> HashMap<(String, String), ReturnDomain> {
    ...
    // 4) Финализируем в (owner_type, method_name) -> ReturnDomain
    let mut out: HashMap<(String, String), ReturnDomain> = HashMap::new();
    for (idx, node) in nodes.iter().enumerate() {
        out.insert(
