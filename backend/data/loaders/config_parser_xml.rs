...
impl ConfigParserXml {
    ...
    fn create_resolution(&self, object: MetadataObject, kind: &MetadataKind) -> TypeResolution {
        ...
        let domain_kind = match kind {
            MetadataKind::Catalog => bsl_shared::domain::types::MetadataKind::Catalog,
            MetadataKind::Document => bsl_shared::domain::types::MetadataKind::Document,
            MetadataKind::InformationRegister => {
                bsl_shared::domain::types::MetadataKind::Register
            }
            MetadataKind::Enum => bsl_shared::domain::types::MetadataKind::Enum,
        };

        // Create configuration type
        let config_type = bsl_shared::domain::types::ConfigurationType {
            kind: domain_kind,
        ...
        TypeResolution {
            ...
            active_facet: Some(bsl_shared::domain::types::FacetKind::Manager),
            available_facets: self.get_facets_for_kind(kind),
        }
    }

    fn get_facets_for_kind(&self, kind: &MetadataKind) -> Vec<bsl_shared::domain::types::FacetKind> {
