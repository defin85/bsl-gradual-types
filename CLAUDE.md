...
# Генерация визуализаций системы типов
cargo run --example generate_category_tree       # Интерактивная визуализация с категориями
cargo run --example visualize_parser_v3         # Визуализация данных парсера
...
Проект предоставляет интерактивные HTML визуализации с правильной иерархией языковых конструкций:

```bash
# Правильная иерархия (как в синтакс-помощнике)
cargo run --example generate_proper_hierarchy

# Структура:
📚 Общее описание встроенного языка
├─ 🔧 Операторы и синтаксические конструкции (struct_*, operator_*)
├─ 📝 Определения (def_*)
├─ 🔤 Выражения (root_*)
└─ 📦 Глобальный контекст (функции, объекты)
```

Подробнее см. [docs/VISUALIZATION.md](docs/VISUALIZATION.md)
