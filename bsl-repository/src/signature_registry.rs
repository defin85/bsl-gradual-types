            ...
            for platform_type in &types {
                for method in &platform_type.methods {
                    ...
                    if let Some(base_type) =
                        bsl_types::facet_utils::extract_placeholder_base_type(&platform_type.name)
                    {
...
use bsl_types::types::{FacetKind, ParameterInfo, RawMethodData};
...
) -> crate::signature_index::ConstructorSignature {
    let params = method
        .params
        .iter()
        .map(|p| ParameterInfo {
...
#[cfg(test)]
#[path = "signature_registry/tests.rs"]
mod tests;
