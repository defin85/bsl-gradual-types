...
<code_editing_rules>
  ...
  <tools_and_process>
    ...
    - По явному требованию «одним патчем»: используй один вызов встроенного `apply_patch` с единым diff‑конвертом — все секции `*** Add/Update/Delete/Move to` должны быть внутри одного блока `*** Begin Patch` ... `*** End Patch`. Не вызывaй `apply_patch` через shell/heredoc. Если объём не влезает в один конверт — остановись и согласуй разбивку на 2–3 крупных патча без самовольного дробления.
