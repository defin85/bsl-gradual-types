...
use crate::core::types::{TypeResolution, FacetKind, Method, Property};
use super::core::{
...
impl PlatformDocumentationProvider {
    ...
    pub async fn initialize_with_platform_config(&self, config: PlatformProviderConfig) -> Result<()> {
    ...
    pub async fn get_types_by_category(&self, category_name: &str) -> Result<Vec<TypeDocumentationFull>> {
    ...
    async fn convert_syntax_node_to_documentation(&self, node: &crate::adapters::syntax_helper_parser::SyntaxNode) -> Result<TypeDocumentationFull> {
        ...
        match node {
            SyntaxNode::Type(type_info) => {
                ...
                let platform_type = PlatformType {
                    ...
                    properties: self.convert_properties(&type_info.structure.properties).await?,
                };
                ...
                let methods = self.convert_methods_full(&type_info.structure.methods).await?;
                
                // Конвертируем свойства в полную документацию  
                let properties = self.convert_properties_full(&type_info.structure.properties).await?;
                
                // Конвертируем примеры
                let examples = type_info.documentation.examples.iter()
                    .map(|ex| CodeExample {
                        title: ex.description.clone().unwrap_or_else(|| "Пример использования".to_string()),
                        code: ex.code.clone(),
                ...
                let availability = type_info.documentation.availability.iter()
                ...
                Ok(TypeDocumentationFull {
                    ...
                    ui_metadata: UiMetadata {
                        ...
                        expanded: false,
                        sort_weight: 0,
                        css_classes: vec![format!("platform-type"), format!("facet-{:?}", type_info.metadata.default_facet.unwrap_or(FacetKind::Collection))],
                    },
                })
            }
            
            _ => {
                Err(anyhow::anyhow!("Cannot convert non-Type SyntaxNode to TypeDocumentationFull"))
            }
        }
    }
    ...
    async fn build_platform_root_category(&self) -> Result<RootCategoryNode> {
        ...
        let mut categories_map: std::collections::HashMap<String, Vec<_>> = std::collections::HashMap::new();
            ...
            match node {
                SyntaxNode::Type(type_info) => {
                    ...
                    let category_name = if type_info.identity.category_path.is_empty() {
                        ...
                        type_info.identity.category_path.split('/').next()
                            .unwrap_or("Без категории").to_string()
                    };
                    
                    categories_map.entry(category_name)
                        .or_default()
                        .push((path.clone(), type_info.clone()));
                }
                SyntaxNode::Category(cat_info) => {
                    // Добавляем информацию о категории
                    categories_map.entry(cat_info.name.clone())
                        .or_default();
                }
                _ => {}
            }
            ...
            if !types.is_empty() {
                let category_node = SubCategoryNode {
                    ...
                    statistics: CategoryStatistics {
                        ...
                        total_methods_count: types.iter().map(|(_, t)| t.structure.methods.len()).sum(),
                        total_properties_count: types.iter().map(|(_, t)| t.structure.properties.len()).sum(),
                        most_popular_type: types.first().map(|(_, t)| t.identity.russian_name.clone()),
                    },
                };
...
impl DocumentationProvider for PlatformDocumentationProvider {
        ...
        for (key, type_doc) in cache.iter() {
            if type_doc.russian_name.contains(type_id) || 
               ...
               type_id.contains(&type_doc.russian_name) {
                println!("✅ Найдено по названию: {} -> {}", type_id, type_doc.russian_name);
    ...
    async fn get_statistics(&self) -> Result<ProviderStatistics> {
        ...
        let memory_mb = (types_count * 1024 + total_methods * 256 + total_properties * 128) as f64 / (1024.0 * 1024.0);
...
impl PlatformDocumentationProvider {
    ...
    async fn convert_properties(&self, property_names: &[String]) -> Result<Vec<Property>> {
        ...
        Ok(property_names.iter().map(|name| Property {
            name: name.clone(),
    ...
    async fn convert_methods_full(&self, method_names: &[String]) -> Result<Vec<MethodDocumentation>> {
        // TODO: Загрузить полную информацию о методах включая параметры и примеры
        Ok(method_names.iter().map(|name| {
    ...
    async fn convert_properties_full(&self, property_names: &[String]) -> Result<Vec<PropertyDocumentation>> {
        Ok(property_names.iter().map(|name| {
    ...
    fn parse_availability_context(&self, availability: &str) -> Option<AvailabilityContext> {
        match availability.to_lowercase().as_str() {
            ...
            "внешнее соединение" | "external connection" => Some(AvailabilityContext::ExternalConnection),
            "мобильное приложение" | "mobile app" => Some(AvailabilityContext::MobileApp),
            "мобильный сервер" | "mobile server" => Some(AvailabilityContext::MobileServer),
