...
**Статус:** 🔴 ПЛАН  
**Цель:** локальный MCP-сервер (stdio), который читает workspace и отдаёт LLM “IDE‑grade” семантический контекст по проекту: диагностики, типы, переходы по символам и компактный `context.pack` в рамках бюджета.
