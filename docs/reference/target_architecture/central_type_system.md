# ⚠️ УСТАРЕВШАЯ ДОКУМЕНТАЦИЯ: CentralTypeSystem

> **ВНИМАНИЕ**: Эта документация устарела. `CentralTypeSystem` был заменён на `SystemCoordinator` с упрощённой архитектурой.
> 
> Актуальная документация: см. `simplified_architecture.md` и реализацию в `backend/src/system/system_coordinator.rs`

---

## Историческая справка: архитектурные роли CentralTypeSystem

**Устаревший** CentralTypeSystem выполнял 5 архитектурных ролей:

🏭 **IoC Container** - создаёт все зависимости и связывает их  
🎛️ **System Coordinator** - координирует инициализацию слоёв  
🏥 **Health Monitor** - проверяет здоровье компонентов  
