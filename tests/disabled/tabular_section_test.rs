#[cfg(test)]
mod tests {
    use bsl_backend::data::loaders::config_parser_xml::ConfigParserXml;
    ...
    fn test_parse_tabular_sections() {
        ...
        let doc_resolution = resolutions.iter().find(|r| {
        ...
        assert!(
            doc_resolution.is_some(),
            "Должен найти документ ЗаказНаряды"
        );
        ...
        if let ResolutionResult::Concrete(ConcreteType::Configuration(cfg)) = &doc.result {
            ...
            println!(
                ...
                cfg.tabular_sections.len()
            );

            // Проверяем табличную часть "Стороны"
            let ts_storony = cfg.tabular_sections.iter().find(|ts| ts.name == "Стороны");
            ...
            let attr_storona = ts_storony.attributes.iter().find(|a| a.name == "Сторона");

            assert!(
                attr_storona.is_some(),
                "Должен быть атрибут Сторона в табличной части"
            );
            ...
            assert!(attr
                .types
                .contains(&"СправочникСсылка.Контрагенты".to_string()));
            assert!(attr
                .types
                .contains(&"СправочникСсылка.Организации".to_string()));
            assert!(attr.types.contains(&"Строка".to_string()));

            // Проверяем табличную часть "Работы"
            let ts_raboty = cfg.tabular_sections.iter().find(|ts| ts.name == "Работы");
            ...
            assert!(
                cfg.tabular_sections.len() >= 2,
                "Должно быть минимум 2 табличные части"
            );
        } else {
