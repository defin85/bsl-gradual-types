...
- synthetic `Averaged` trace явно помечается как synthetic и не выдаёт trustworthy `v8` pre-method attribution provenance, `v9` pre-service-scope split или `v10` dispatch split за per-request факт;
- при более старом backend payload `v9`/`v8`/`v7` extension деградирует явно и не выдумывает отсутствующие `v10`/`v9`/`v8` поля;
