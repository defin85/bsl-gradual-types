...
#[test]
fn infers_return_type_through_local_helper_call() {
    let tmp = TempDir::new().unwrap();
    let module_path = tmp
        .path()
        .join("CommonModules")
...
#[test]
fn infers_return_type_across_modules_through_manager_call() {
    let tmp = TempDir::new().unwrap();
    let manager_path = tmp
        .path()
        .join("Catalogs")
        .join("Контрагенты")
        .join("Ext")
...
"#,
    ...
    write(
        ...
        r#"
Функция Ф1() Экспорт
    Возврат Справочники.Контрагенты.ФМ();
КонецФункции
"#,
    );
    ...
    obj.manager_module_path = Some(manager_path);
    ...
    cm.common_module_path = Some(common_path.clone());
    cm.common_module_properties = Some(CommonModuleProperties {
        server: true,
        client_managed_application: true,
        client_ordinary_application: false,
    ...
    let indexed = index_configuration_bsl_modules(tmp.path(), &[obj, cm]).unwrap();
    let sig = indexed
        .config_methods
        .iter()
        .find(|(t, m)| t == "ОбщиеМодули.ОбщийМодуль1" && m.name == "Ф1")
