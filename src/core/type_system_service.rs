...
use super::unified_type_system::{
    UnifiedTypeSystem, UnifiedSystemConfig, LspTypeInterface, WebTypeInterface,
    TypeDisplayInfo, TypeDetailedInfo
};
...
impl TypeSystemService {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        self.set_stage(InitializationStage::LoadingPlatformTypes, "Инициализация UnifiedTypeSystem...").await;
        
        match self.unified_system.initialize().await {
            Ok(_) => {
                self.set_stage(InitializationStage::Ready, "TypeSystemService готов к работе").await;
            ...
            Err(e) => {
                self.set_stage(InitializationStage::Failed, &format!("Ошибка инициализации: {}", e)).await;
                Err(e)
            }
        }
    }
    ...
    pub async fn get_variable_type(&self, variable_name: &str, context: &str) -> TypeResolution {
        self.increment_lsp_requests().await;
        self.lsp_interface.get_variable_type(variable_name, context).await
    }
    
    /// Проверить совместимость типов (для LSP)
    pub async fn check_assignment_compatibility(&self, from_type: &TypeResolution, to_type: &TypeResolution) -> bool {
        self.increment_lsp_requests().await;
        self.lsp_interface.check_assignment_compatibility(from_type, to_type).await
    }
    ...
    pub async fn get_type_hierarchy(&self) -> Result<TypeHierarchy> {
        ...
        Err(anyhow::anyhow!("get_type_hierarchy интеграция в разработке"))
    }
    ...
    pub async fn get_performance_stats(&self) -> Result<PerformanceStats> {
        ...
        let cache_hit_ratio = if unified_stats.cache_hits + unified_stats.cache_misses > 0 {
            unified_stats.cache_hits as f64 / (unified_stats.cache_hits + unified_stats.cache_misses) as f64
        } else {
            0.0
        };
...
impl InitializationState {
    ...
    fn complete_initialization(&mut self) {
        ...
        if let Some(start_time) = self.start_time {
            ...
            self.status_message = format!("Инициализация завершена за {:.2}s", duration.as_secs_f64());
        } else {
