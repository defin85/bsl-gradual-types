...
- [x] 1.1 Define the transport-runtime contract for completion admission after the
...
- [x] 1.2 Define the additive completion timeline / incident-bundle contract for truthful
...
- [x] 1.3 Define the exact publication point for same-file ingress tokens:
      only after current-revision handoff is registered for that file/version, not at
      dispatcher-event emission time.

## 2. Implementation

- [x] 2.1 Export the new completion pre-dispatch decomposition fields and update incident-bundle /
...
- [x] 2.2 Introduce explicit same-file document ingress ownership/token publication that preserves
...
- [x] 2.3 Rework completion admission so first response depends on the relevant same-file ingress
...
- [x] 2.4 Split the transport runtime into independently progressing reader, single-owner
...
- [x] 3.1 Add backend regressions proving a stalled scheduler branch cannot stop transport reader
      progress, late cancel classification, or ready response output progression.
- [x] 3.2 Add reader-backpressure regressions proving local spillover wait before `adapter_read`
      is explicitly attributed and cannot silently masquerade as client-side ingress.
- [x] 3.3 Add same-file transport-path regressions proving `didChange`/`didSave` ingress ownership
...
- [x] 3.4 Refresh representative live/perf evidence on `examples/conf_big` showing that the new
...
- [x] 3.5 Attach one correlation slice for the worst representative outlier:
...
- [x] 4.1 Run targeted backend/runtime/transport/perf/incident-bundle regressions for task
      isolation, reader backpressure attribution, same-file ingress ownership, and truthful
      pre-dispatch decomposition.
- [x] 4.2 Run `openspec validate refactor-47-completion-transport-runtime-isolation --strict --no-interactive`.
