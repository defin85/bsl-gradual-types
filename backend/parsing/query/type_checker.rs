...
use bsl_shared::domain::context::ContextResolver;
...
impl QueryTypeChecker {
    ...
    pub fn check_query(&mut self, query: &Query) -> QueryResult {
        ...
        for field in &query.select_clause.fields {
            ...
            let field_name = field
                .alias
                .clone()
                .unwrap_or_else(|| self.expression_to_string(&field.expression));
    ...
    fn load_table_schema(
        ...
        let schema = match table_ref {
            TableReference::Catalog(_, name) => self.load_catalog_schema(name),
            TableReference::Document(_, name) => self.load_document_schema(name),
            TableReference::Register(reg_type, name) => self.load_register_schema(reg_type, name),
            TableReference::VirtualTable(base, vt_name, params) => {
                self.load_virtual_table_schema(base, vt_name, params)
            }
            TableReference::Table(name) => self.load_generic_table_schema(name),
        ...
        let table_name = alias
            .map(|s| s.to_string())
            .unwrap_or_else(|| self.table_reference_to_string(table_ref));
    ...
    fn load_catalog_schema(&self, name: &str) -> TableSchema {
        ...
        fields.insert(
            ...
            FieldSchema {
                ...
                type_resolution: TypeResolution::known(ConcreteType::Configuration(
                    ConfigurationType {
        ...
        fields.insert(
            ...
            FieldSchema {
                name: "Код".to_string(),
                type_resolution: TypeResolution::known(ConcreteType::Primitive(
    ...
    fn load_virtual_table_schema(
        &self,
        _base: &str,
        vt_name: &str,
        _params: &[VirtualTableParameter],
    ) -> TableSchema {
    ...
    fn check_expression(
        ...
        match expr {
            Expression::Field(name) => self.resolve_field_type(None, name, errors),
            ...
            Expression::Function(func) => self.check_function_call(func, errors),
            Expression::BinaryOp(left, op, right) => self.check_binary_op(left, op, right, errors),
            Expression::UnaryOp(op, expr) => self.check_unary_op(op, expr, errors),
            ...
            Expression::Case(case_expr) => self.check_case_expression(case_expr, errors),
    ...
    fn literal_type(&self, lit: &Literal) -> TypeResolution {
        match lit {
            ...
            Literal::Null | Literal::Undefined => {
                TypeResolution::known(ConcreteType::Special(SpecialType::Undefined))
            }
            Literal::EmptyReference => {
                TypeResolution::known(ConcreteType::Special(SpecialType::Null))
            }
        }
    }
    ...
    fn check_function_call(
        ...
        match func.name.to_uppercase().as_str() {
            ...
            "СТРОКА" => TypeResolution::known(ConcreteType::Primitive(PrimitiveType::String)),
            "ДАТАВРЕМЯ" => {
    ...
    fn validate_group_by(
        ...
        for field in &select.fields {
            if !self.is_aggregate_expression(&field.expression)
                && !self.is_in_group_by(&field.expression, group_by)
            {
                errors.push(QueryError {
                    message: format!(
    ...
    fn is_in_group_by(&self, expr: &Expression, group_by: &GroupByClause) -> bool {
        ...
        group_by
            .fields
            .iter()
            .any(|gb_expr| self.expression_to_string(gb_expr) == expr_str)
    }
...
mod tests {
    ...
    fn test_simple_query_type_check() {
        let mut checker = QueryTypeChecker::new(ContextResolver::new());
        ...
        let query = Query {
            ...
            from_clause: FromClause {
                sources: vec![TableSource {
                    table: TableReference::Document(
                        "Документ".to_string(),
                        "ПоступлениеТоваровУслуг".to_string(),
                    ),
                    alias: None,
                    joins: vec![],
                }],
