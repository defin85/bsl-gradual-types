...
fn test_catalog_kontragenty() {
    ...
    let metadata = parser.get_discovered_metadata("Справочники.Контрагенты").unwrap();
    
    assert_eq!(metadata.name, "Контрагенты");
    assert_eq!(metadata.kind, MetadataKind::Catalog);
    assert_eq!(metadata.uuid, Some("e3cbde3c-52ed-494d-bd38-ed777a837517".to_string()));
...
fn test_register_attributes() {
    ...
    let metadata = parser.get_discovered_metadata("РегистрыСведений.ТестовыйРегистрСведений").unwrap();
