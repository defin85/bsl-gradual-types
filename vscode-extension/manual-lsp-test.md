...
- `Server Timeline` на payload `version=9` показывает bounded root-cause attribution (`service_future_created_at_ms`, `transport_to_service_future_wait_ms`, `service_future_to_scope_wait_ms`, существующие `transport_to_service_scope_wait_ms`, `service_scope_to_method_wait_ms`, bounded `pre_method_attribution_provenance`, `dispatcher_resolution_latency_ms`, `prepare_progress`, `wait_for_file_version_runtime`, `snapshot_with_deps_runtime`, `snapshot_with_deps_timeout_runtime`, `timeout_attribution`, bounded `exact_wait` waiter/task-state и `artifact_poll`);
...
- synthetic `Averaged` trace явно помечается как synthetic и не выдаёт trustworthy `v8` pre-method attribution provenance или `v9` pre-service-scope split за per-request факт;
- при более старом backend payload `v8`/`v7` extension деградирует явно и не выдумывает отсутствующие `v9`/`v8` поля;
- export bundle сохраняет `summary.md`, `incident.json` и `raw/*` attachments без использования truncated Output dump как источника;
- `incident.json` и `summary.md` выражают request-centric handoff: `capture_scope`, `request_count`, bounded request list, bounded `service_future_created` split и explicit `correlation=correlated|unavailable|ambiguous` без guessed probe/trace pairs.
