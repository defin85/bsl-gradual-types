...
use super::domain::{TypeResolutionService, TypeContext, CompletionItem, CompletionKind, TypeSearchResult};
use super::data::{TypeRepository, RawTypeData, TypeSource};
...
impl LspTypeService {
    ...
    pub async fn resolve_at_position(&self, file_path: &str, line: u32, column: u32, expression: &str) -> TypeResolution {
        ...
        let resolution = self.resolution_service.resolve_expression(expression, &context).await;
        
        // Кешируем результат
        {
            let mut cache = self.lsp_cache.write().await;
            cache.position_cache.insert(position_key, resolution.clone());
        }
    ...
    pub async fn get_completions_fast(&self, prefix: &str, file_path: &str, line: u32, column: u32) -> Vec<LspCompletion> {
        let start_time = std::time::Instant::now();
        ...
        let completions = self.resolution_service.get_completions(prefix, &context).await;
        
        // Конвертируем в LSP формат
        let lsp_completions: Vec<LspCompletion> = completions.into_iter()
        ...
        {
            let mut cache = self.lsp_cache.write().await;
            cache.completion_cache.insert(cache_key, lsp_completions.clone());
        }
    ...
    pub async fn get_hover_info(&self, expression: &str, file_path: &str, line: u32, column: u32) -> Option<HoverInfo> {
        // Проверяем кеш hover
        ...
        // Разрешаем тип
        let resolution = self.resolve_at_position(file_path, line, column, expression).await;
    ...
    pub async fn check_assignment_compatibility(&self, from_expr: &str, to_expr: &str, context: &TypeContext) -> bool {
        let from_type = self.resolution_service.resolve_expression(from_expr, context).await;
        let to_type = self.resolution_service.resolve_expression(to_expr, context).await;
        
        // Простая проверка совместимости (можно расширить)
        match (&from_type.result, &to_type.result) {
            (crate::core::types::ResolutionResult::Concrete(from_concrete), 
             crate::core::types::ResolutionResult::Concrete(to_concrete)) => {
                // Конкретные типы должны совпадать
                std::mem::discriminant(from_concrete) == std::mem::discriminant(to_concrete)
            }
            _ => {
                // Для остальных случаев пока разрешаем (можно ужесточить)
                true
            }
        }
    }
...
impl WebTypeService {
    ...
    pub async fn get_all_types_with_documentation(&self) -> Result<Vec<WebTypeInfo>> {
        ...
        println!("✅ Подготовлено {} типов для веб", web_types.len());
        Ok(web_types)
    }
    ...
    pub async fn build_type_hierarchy(&self) -> Result<WebTypeHierarchy> {
        ...
        for raw_type in all_types {
            for category in &raw_type.category_path {
                categories_map.entry(category.clone())
        ...
        println!("✅ Иерархия построена: {} категорий, {} типов", 
                hierarchy.categories.len(), hierarchy.total_types);
        
        Ok(hierarchy)
    }
    
    /// Расширенный поиск для веб-интерфейса
    pub async fn advanced_search(&self, query: &str, filters: SearchFilters) -> Result<Vec<WebSearchResult>> {
        ...
        let web_results = filtered_results.into_iter()
            .map(|result| WebSearchResult {
                type_name: result.raw_data.russian_name.clone(),
                category: result.raw_data.category_path.first()
                    .unwrap_or(&"Неопределено".to_string()).clone(),
                description: result.raw_data.documentation.clone(),
                relevance_score: result.relevance_score,
                match_highlights: result.match_highlights.iter()
                    .map(|span| span.text.clone())
                    .collect(),
                url: format!("/types/{}", urlencoding::encode(&result.raw_data.russian_name)),
            })
            .collect();
    ...
    pub async fn get_type_details(&self, type_name: &str) -> Result<WebTypeDetails> {
        // TODO: Получить тип через публичный API TypeResolutionService
        // Заглушка пока нет публичного API
        Ok(WebTypeDetails {
            basic_info: WebTypeInfo {
                name: type_name.to_string(),
                description: "Описание отсутствует".to_string(),
                methods_count: 0,
                properties_count: 0,
                examples: Vec::new(),
                url: format!("/types/{}", urlencoding::encode(type_name)),
                tags: Vec::new(),
                },
                methods: Vec::new(), // Заглушка
                properties: Vec::new(), // Заглушка
    ...
    async fn apply_search_filters(&self, results: Vec<TypeSearchResult>, _filters: &SearchFilters) -> Result<Vec<TypeSearchResult>> {
...
impl AnalysisTypeService {
    ...
    pub async fn analyze_project(&self, project_path: &Path) -> Result<ProjectAnalysisResult> {
        ...
        let coverage_report = self.coverage_calculator.calculate_coverage(&bsl_files).await?;
    ...
    pub async fn calculate_type_coverage(&self, files: &[std::path::PathBuf]) -> Result<CoverageReport> {
        self.coverage_calculator.calculate_coverage(files).await
    }
    
    /// Найти ошибки типов в файлах
    pub async fn find_type_errors(&self, files: &[std::path::PathBuf]) -> Result<Vec<TypeDiagnostic>> {
...
mod tests {
    use super::*;
    use crate::ideal::data::{InMemoryTypeRepository, TypeSource, ParseMetadata};
    ...
    async fn test_lsp_type_service() {
        ...
        let resolution = lsp_service.resolve_at_position("test.bsl", 10, 5, "Массив").await;
        assert_ne!(resolution.certainty, crate::core::types::Certainty::Known); // Без данных будет Unknown
        
        // Тестируем автодополнение
        let completions = lsp_service.get_completions_fast("Стр", "test.bsl", 10, 5).await;
    ...
    async fn test_web_type_service() {
        ...
        let web_types = web_service.get_all_types_with_documentation().await.unwrap();
