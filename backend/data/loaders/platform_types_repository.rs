...
impl PlatformTypesRepository {
    ...
    fn create_standard_type(&self, type_name: &str) -> TypeResolution {
        use bsl_shared::domain::types::PrimitiveType;
    ...
    fn resolve_member_access(&self, base: &str, member: &str) -> TypeResolution {
        use bsl_shared::domain::types::{ConfigurationType, FacetKind, MetadataKind};
...
impl PlatformTypesRepository {
    ...
    pub fn get_completions(&self, _expression: &str) -> Vec<bsl_shared::domain::CompletionItem> {
        // Базовая реализация - возвращаем все глобальные объекты как варианты
        let globals = self.get_platform_globals_enhanced();
        globals
            .keys()
            .map(|name| bsl_shared::domain::CompletionItem {
                label: name.clone(),
                kind: bsl_shared::domain::CompletionKind::Global,
