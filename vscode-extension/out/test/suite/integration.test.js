...
КонецПроцедуры`;
    ...
    test('Language Server communication', async function () {
        ...
        const commands = await vscode.commands.getCommands();
        assert.ok(commands.includes('bslAnalyzer.restartServer'), 'LSP restart command should be available');
    ...
    test('1C Configuration handling', async () => {
        ...
        if (platformVersion) {
            ...
            assert.ok(versionRegex.test(platformVersion), `Platform version should match X.X.X format, got: ${platformVersion}`);
        }
        // Если путь к конфигурации указан, проверяем его валидность
        if (configPath && configPath !== '') {
            // Путь должен быть абсолютным
            assert.ok(path.isAbsolute(configPath), 'Configuration path should be absolute');
        }
    });
    /**
...
suite('Diagnostics Test Suite', () => {
    test('Diagnostics provider should be registered', async () => {
        ...
        const commands = await vscode.commands.getCommands();
        assert.ok(commands.includes('bslAnalyzer.showMetrics'), 'Show metrics command should exist');
        assert.ok(commands.includes('bslAnalyzer.generateReports'), 'Generate reports command should exist');
    });
...
suite('WebView Test Suite', () => {
    test('WebView commands should be registered', async () => {
        ...
        for (const cmd of webViewCommands) {
            assert.ok(commands.includes(cmd), `WebView command ${cmd} should be registered`);
        }
    });
});
/**
...
suite('Compatibility Test Suite', () => {
    test('Extension should work with current VSCode version', () => {
        ...
        assert.ok(major > 1 || (major === 1 && minor >= 75), `VSCode version ${vscodeVersion} should be >= 1.75.0`);
    });
