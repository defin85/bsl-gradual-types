#[cfg(test)]
mod tests {
    use bsl_gradual_types::parser::{BslParser, Statement, Expression};
    ...
    fn test_parse_simple_program() {
        ...
        match &program.statements[2] {
            Statement::FunctionDecl { name, params, return_value, .. } => {
    ...
    fn test_parse_platform_types() {
        ...
        match &program.statements[2] {
            Statement::Assignment { value, .. } => {
                assert!(matches!(value, Expression::New { type_name, .. } if type_name == "Запрос"));
            }
            _ => panic!("Ожидалось присваивание"),
        }
    }
