...
use crate::core::types::{
    ...
    WeightedType, ResolutionSource, ResolutionMetadata, PrimitiveType
};
...
impl UnionTypeManager {
    ...
    pub fn is_compatible_with_union(type_: &TypeResolution, union_types: &[WeightedType]) -> bool {
        if let ResolutionResult::Concrete(concrete) = &type_.result {
            return union_types.iter().any(|ut| Self::types_compatible(concrete, &ut.type_));
        }
...
mod tests {
    ...
    fn test_create_simple_union() {
        ...
        match union.result {
            ResolutionResult::Union(union_types) => {
                ...
                assert!(union_types.iter().any(|wt| wt.type_ == ConcreteType::Primitive(PrimitiveType::String)));
                assert!(union_types.iter().any(|wt| wt.type_ == ConcreteType::Primitive(PrimitiveType::Number)));
            }
    ...
    fn test_is_compatible_with_union() {
        ...
        assert!(UnionTypeManager::is_compatible_with_union(&string_type, &union_types));
        ...
        assert!(!UnionTypeManager::is_compatible_with_union(&boolean_type, &union_types));
    }
