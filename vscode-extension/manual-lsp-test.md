...
- synthetic `Averaged` trace явно помечается как synthetic и не выдаёт trustworthy `v8` pre-method attribution provenance, `v9` pre-service-scope split, `v10` dispatch split или `v11` first-poll / first-wake split за per-request факт;
- при более старом backend payload `v10`/`v9`/`v8`/`v7` extension деградирует явно и не выдумывает отсутствующие `v11`/`v10`/`v9`/`v8` поля;
