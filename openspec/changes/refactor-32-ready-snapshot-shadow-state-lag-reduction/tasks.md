...
- [x] 1.1 Rework same-file exact ready-snapshot producer policy so a still-current `didSave`
...
- [x] 1.2 Preserve truthful latest-wins supersession, cancellation, and fallback semantics when a
      newer same-file revision or save cycle overtakes the current target.

## 2. Parser-base lag reduction for ranged `didChange`

- [x] 2.1 Keep a parser-base-capable exact head close enough to `shadow_state` during ranged
...
- [x] 2.2 Bound retarget/restart waste so the newest same-file revision prefers advancing one
...
- [x] 3.1 Add targeted backend/runtime regressions covering representative same-file
...
- [x] 3.2 Refresh representative `conf_big` live evidence / incident-bundle proof showing that the
...
- [x] 4.1 Run targeted backend tests and representative live repros for the new exact-head
      freshness contract.
- [x] 4.2 Run `openspec validate refactor-32-ready-snapshot-shadow-state-lag-reduction --strict
      --no-interactive`.
