...
fn test_methods_limit_respected() {
    ...
    // Проверка 2: Показаны первые 10 методов в лексикографическом порядке
    assert!(hover.contains("Метод0"), "Должен быть Метод0");
    assert!(hover.contains("Метод10"), "Должен быть Метод10");

    // Проверка 3: НЕ показаны методы за пределами лимита
    assert!(
        !hover.contains("Метод18"),
        "Метод18 не должен отображаться (за пределами лимита)"
    );
    assert!(
