...
Архитектурный драйвер P9: после стабилизации включить **единый путь вычислений** для LSP и удалить/изоляционно отрезать legacy.
...
2) **Legacy кэши (`IrCache`/`AnalysisCache`) не участвуют в LSP v2**:
   - либо удалить полностью (если нет нужных клиентов),
   - либо оставить только для CLI/Web, но физически разорвать зависимости “LSP → legacy cache”.
3) **TypeSystemService**:
   - **решение:** считать `TypeSystemService` legacy‑API для CLI/Web на время миграции;
     LSP (после P9) не должен зависеть от `TypeSystemService` вообще.
   - дальнейшая миграция: добавить v2‑фасад/entrypoints поверх `AnalysisV2 + DepsBundleV2` и перевести CLI/Web на него;
     после этого удалить legacy реализацию (и связанные кэши), либо оставить только доказанно полезные части вне hot path.
...
- [ ] Зафиксировать “кто клиенты legacy”: LSP / Web API / CLI (что именно использует `TypeSystemService`, `IrCache`, `AnalysisCache`).
- [ ] Составить список LSP entrypoints, которые ещё используют legacy:
  - [ ] `rg -n "TypeSystemService" backend/src/bin/lsp_server -S`
  - [ ] `rg -n "parse_to_ir" backend/src/bin/lsp_server -S`
  - [ ] `rg -n "IrCache|AnalysisCache" backend/src/bin/lsp_server -S`

### 1) Перевести `goto_definition` на v2 (и добить остальные LSP endpoints)

- [ ] `textDocument/definition` (`goto_definition`): реализовать v2 путь:
  - [ ] вход: `(FileId, file_version, position)` + `(AnalysisV2 snapshot, deps bundle)`,
  - [ ] получать IR/семантику только из v2 queries,
  - [ ] сохранить текущую семантику разрешения (конфигурационные типы / user-defined / platform).
...
- [ ] `backend/src/bin/lsp_server/server/core.rs`: убрать чтение env `BSL_INTELLISENSE_V2_SALSA`.
- [ ] `backend/src/bin/lsp_server/server/mod.rs`: удалить поле `use_salsa_v2`.
- [ ] `backend/src/bin/lsp_server/server/language_server.rs`: удалить legacy ветки `if self.use_salsa_v2 { ... } else { ... }`.
- [ ] (Если нужно) временно заменить runtime flag на compile-time feature `legacy-lsp` (чтобы проще чистить код), затем удалить и его.
...
- [ ] LSP server не должен:
  ...
  - [ ] использовать `ParserCoordinator::parse_to_ir` в запросах.
- [ ] Решение по `IrCache`/`AnalysisCache`:
  - [ ] либо удалить полностью (если не нужно),
  - [ ] либо оставить только для CLI/Web, явно пометив как legacy и убрав все ссылки из LSP кода.
...
- [ ] Принять решение по судьбе `TypeSystemService`:
  - [x] решение: `TypeSystemService` остаётся legacy‑API для Web/CLI до полной миграции на v2;
        LSP после P9 не использует `TypeSystemService`.
  - [ ] реализовать v2‑фасад/entrypoints для CLI/Web и после миграции удалить legacy реализацию.
- [ ] После миграции CLI/Web: удалить `IrCache`/`AnalysisCache` (или оставить только там, где есть доказанная польза и корректность).
