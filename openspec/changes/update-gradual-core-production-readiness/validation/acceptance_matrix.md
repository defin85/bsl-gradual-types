...
| --- | --- | --- | --- | --- |
| OpenSpec | `openspec validate update-gradual-core-production-readiness --strict --no-interactive` | Change validates strictly with no errors | Any strict validation failure in change files or specs | `openspec/changes/update-gradual-core-production-readiness/tasks.md` |
...
| Final readiness honesty | Current status review | Current verdict stays `partial` until traceability/closure refresh is done and critical backlog is closed | Any wording or status implies `ready/complete` while `6mx.6` remains open | `openspec/changes/update-gradual-core-production-readiness/validation/readiness-review-status.md` |
