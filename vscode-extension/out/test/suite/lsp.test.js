...
suite('LSP Integration Test Suite', () => {
    ...
    test('Commands should not be registered twice', async function () {
        ...
        for (const [cmd, count] of commandCounts) {
            assert.strictEqual(count, 1, `Command ${cmd} registered ${count} times (should be 1)`);
        }
    });
    /**
    ...
    test('Extension commands should be registered before LSP starts', async () => {
        ...
        for (const cmd of extensionCommands) {
            assert.ok(commands.includes(cmd), `Extension command ${cmd} should be registered`);
        }
    });
    /**
    ...
    test('Server restart should not cause duplicate commands', async function () {
        ...
        try {
            ...
            assert.strictEqual(countAfter, countBefore, `Command count changed after restart: ${countBefore} -> ${countAfter}`);
        }
        catch (error) {
    ...
    test('Should handle LSP initialization errors gracefully', async () => {
        ...
        const commands = await vscode.commands.getCommands();
        assert.ok(commands.includes('bslAnalyzer.searchType'), 'Basic commands should work even without LSP');
    });
    /**
    ...
    test('Extension and LSP commands should be properly isolated', async () => {
        ...
        assert.ok(extensionCommands.length > 0, 'Should have extension-specific commands');
        // LSP команды могут отсутствовать в тестовой среде
        assert.ok(lspCommands.length >= 0, 'LSP commands count should be non-negative');
    });
});
/**
 * Тесты для MILESTONE 2.20.1 — LSP Server Status Indicator
 */
suite('LSP Server Status Indicator Test Suite', () => {
    test('updateLspStatus should handle State.Starting', async () => {
        // Функция должна корректно обработать состояние Starting
        // В тестовой среде statusBarItem может быть не инициализирован,
        // поэтому проверяем, что функция не бросает исключений
        try {
            (0, progress_1.updateLspStatus)(node_1.State.Starting);
            assert.ok(true, 'State.Starting handled without errors');
        }
        catch (error) {
            assert.fail(`updateLspStatus failed with State.Starting: ${error}`);
        }
    });
    test('updateLspStatus should handle State.Running', async () => {
        try {
            (0, progress_1.updateLspStatus)(node_1.State.Running);
            assert.ok(true, 'State.Running handled without errors');
        }
        catch (error) {
            assert.fail(`updateLspStatus failed with State.Running: ${error}`);
        }
    });
    test('updateLspStatus should handle State.Stopped', async () => {
        try {
            (0, progress_1.updateLspStatus)(node_1.State.Stopped);
            assert.ok(true, 'State.Stopped handled without errors');
        }
        catch (error) {
            assert.fail(`updateLspStatus failed with State.Stopped: ${error}`);
        }
    });
    test('LSP state changes should update status bar', async function () {
        this.timeout(15000);
        ...
        // Даём время на инициализацию LSP
        await new Promise(resolve => setTimeout(resolve, 3000));
        // Проверяем, что extension активирован (косвенная проверка работы LSP)
...
suite('Multiple Activation Protection Test Suite', () => {
    ...
    test('Commands should have safeguards against double registration', async () => {
        ...
        assert.strictEqual(uniqueCommands.size, bslCommands.length, 'All commands should be unique');
    });
});
