...
exports.TypeTreeBuilder = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
const typeModels_1 = require("./typeModels");
const typeFormatter_1 = require("./typeFormatter");
...
class TypeTreeBuilder {
    ...
    async loadTypes() {
        ...
        try {
            ...
            for (let page = 0; page < 200; page++) {
                const response = await (0, customRequests_1.getAllTypes)({ limit: pageSize, offset });
            ...
            this.outputChannel?.appendLine(`TypeTreeBuilder: Platform=${this.platformTypes.size}, Config=${this.configTypes.size}`);
            // Группируем типы по категориям
            this.categorizeTypes();
            this.outputChannel?.appendLine(`TypeTreeBuilder: Categorized into ${this.typeCategories.size} categories`);
        }
        catch (error) {
    ...
    getPlatformCategories() {
        ...
        for (const [key, category] of this.typeCategories) {
            if (key.startsWith('platform:')) {
                const categoryItem = new typeModels_1.HierarchicalTypeItem(`${category.icon} ${category.name} (${category.types.length})`, vscode.TreeItemCollapsibleState.Collapsed, category.name, 'category', key);
    ...
    getCategoryTypes(categoryKey) {
        ...
        return category.types.slice(0, 100).map(entity => {
            ...
            return new typeModels_1.HierarchicalTypeItem(entity.display_name || entity.qualified_name, hasMembers ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None, entity.qualified_name, 'type', entity.qualified_name);
        });
    }
    /**
    ...
    getTypeMembers(typeName) {
        ...
        if (methodCount > 0) {
            const methodsFolder = new typeModels_1.HierarchicalTypeItem(`📦 Методы (${methodCount})`, vscode.TreeItemCollapsibleState.Collapsed, 'Методы', 'methods-folder', typeName);
        ...
        if (propCount > 0) {
            const propsFolder = new typeModels_1.HierarchicalTypeItem(`📋 Свойства (${propCount})`, vscode.TreeItemCollapsibleState.Collapsed, 'Свойства', 'properties-folder', typeName);
            items.push(propsFolder);
        }
        // Добавляем описание, если есть
        if (entity.documentation) {
            const docItem = new typeModels_1.HierarchicalTypeItem(`📄 ${entity.documentation.substring(0, 100)}...`, vscode.TreeItemCollapsibleState.None, 'Описание', 'documentation');
    ...
    getTypeMethods(typeName) {
        ...
        return Object.entries(entity.interface.methods).slice(0, 50).map(([name, method]) => {
            const item = new typeModels_1.HierarchicalTypeItem(`⚡ ${name}`, vscode.TreeItemCollapsibleState.None, name, 'method');
            item.tooltip = (0, typeFormatter_1.formatMethodTooltip)(name, method);
            return item;
        });
    }
    /**
    ...
    getTypeProperties(typeName) {
        ...
        return Object.entries(entity.interface.properties).slice(0, 50).map(([name, prop]) => {
            const item = new typeModels_1.HierarchicalTypeItem(`📌 ${name}`, vscode.TreeItemCollapsibleState.None, name, 'property');
            item.tooltip = (0, typeFormatter_1.formatPropertyTooltip)(name, prop);
            return item;
        });
    }
    ...
    categorizeTypes() {
        ...
        for (const [categoryName, typePatterns] of Object.entries(typeFormatter_1.PLATFORM_CATEGORIES)) {
            const category = {
                name: categoryName,
                icon: (0, typeFormatter_1.getCategoryIcon)(categoryName),
                types: []
            };
        ...
        if (this.configTypes.size > 0) {
            ...
            for (const [, entity] of this.configTypes) {
                const categoryName = (0, typeFormatter_1.getConfigCategoryName)(entity.entity_kind || 'Other');
...
exports.TypeTreeBuilder = TypeTreeBuilder;
//# sourceMappingURL=typeTreeBuilder.js.map