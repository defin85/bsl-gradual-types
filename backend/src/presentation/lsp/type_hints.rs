...
use bsl_shared::domain::analysis::TypeContext;
...
impl TypeHintsProvider {
                ...
                if self.settings.show_return_types {
                    if let Some(func_sig) = type_context.functions.get(name) {
                        if let Some(return_type) = &func_sig.return_type {
                            if self.should_show_hint(return_type) {
                                hints.push(self.create_return_type_hint(name, return_type, 0, 0));
                            }
                        }
                    }
                }
            ...
            Expression::Call { function, args } => {
                ...
                if let Expression::Identifier(func_name) = &**function {
                    if let Some(func_sig) = type_context.functions.get(func_name) {
                        if let Some(return_type) = &func_sig.return_type {
                            if self.settings.show_return_types && self.should_show_hint(return_type)
                            {
                                hints.push(self.create_function_call_hint(
                                    func_name,
                                    return_type,
                                    0,
                                    0,
    ...
    fn format_type_hint(&self, type_res: &TypeResolution) -> String {
        let base_text = match &type_res.result {
            ResolutionResult::Concrete(ConcreteType::Primitive(primitive)) => match primitive {
                bsl_shared::domain::types::PrimitiveType::String => "Строка",
                bsl_shared::domain::types::PrimitiveType::Number => "Число",
                bsl_shared::domain::types::PrimitiveType::Boolean => "Булево",
                bsl_shared::domain::types::PrimitiveType::Date => "Дата",
            }
            .to_string(),
        ...
        if full_text.len() > self.settings.max_hint_length {
            format!(
                "{}...",
                &full_text[..self.settings.max_hint_length.saturating_sub(3)]
            )
        } else {
            full_text
        }
    ...
    fn format_concrete_type(&self, concrete_type: &ConcreteType) -> String {
        match concrete_type {
            ConcreteType::Primitive(primitive) => match primitive {
                bsl_shared::domain::types::PrimitiveType::String => "Str",
                bsl_shared::domain::types::PrimitiveType::Number => "Num",
                bsl_shared::domain::types::PrimitiveType::Boolean => "Bool",
                bsl_shared::domain::types::PrimitiveType::Date => "Date",
            }
            .to_string(),
    ...
    fn format_detailed_type(&self, type_res: &TypeResolution) -> String {
        match &type_res.result {
            ...
            ResolutionResult::Union(union_types) => {
                let types_info: Vec<String> = union_types
                    .iter()
                    .map(|wt| {
                        format!(
                            "{} ({}%)",
...
impl SemanticTokensProvider {
    ...
    fn create_function_token(
        &self,
        func_name: &str,
        func_sig: &bsl_shared::domain::analysis::type_checker::FunctionSignature,
        line: u32,
        start_char: u32,
    ) -> SemanticToken {
        SemanticToken {
            ...
            token_modifiers_bitset: if func_sig.is_exported {
...
impl TypeHintsIntegration {
    ...
    pub fn handle_inlay_hints_request(
        ...
        self.provider
            .get_inlay_hints(program, type_context, params.range)
    }
    ...
    pub fn handle_semantic_tokens_request(
        ...
        self.semantic_provider
            .get_semantic_tokens(program, type_context)
    }
...
mod tests {
    ...
    fn create_test_context() -> TypeContext {
        ...
        variables.insert(
            "stringVar".to_string(),
            bsl_shared::domain::standard_types::primitive_type(
        ...
        variables.insert(
            "numberVar".to_string(),
            bsl_shared::domain::standard_types::primitive_type(
                bsl_shared::domain::types::PrimitiveType::Number,
            ),
        );
        ...
        functions.insert(
            "TestFunc".to_string(),
            bsl_shared::domain::analysis::type_checker::FunctionSignature {
                name: "TestFunc".to_string(),
                parameters: vec!["param1".to_string()],
                return_type: Some(bsl_shared::domain::standard_types::primitive_type(
                    bsl_shared::domain::types::PrimitiveType::Number,
                )),
                is_function: true,
                is_exported: true,
    ...
    fn test_format_type_hint() {
        ...
        // Тест с Union типом
        let union_type =
            bsl_shared::domain::analysis::union_types::UnionTypeManager::from_concrete_types(vec![
