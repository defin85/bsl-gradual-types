...
impl DerivedArtifactsCache {
    ...
    pub(crate) fn retain_versions_for_file(
        ...
        let mut outcome = TypeIndexStoreOutcome::default();
        if let Some(versioned) = self.by_file.get_mut(&file_id) {
            ...
            for version in versions_to_remove {
                if let Some(removed) = versioned.remove(&version) {
                    let removed_type_indexes = removed.type_index_artifacts_count() as u64;
                    if removed_type_indexes > 0 {
                        self.type_index_artifacts_total = self
                            .type_index_artifacts_total
                            .saturating_sub(removed_type_indexes as usize);
                        outcome.evicted_per_file_window_total = outcome
                            .evicted_per_file_window_total
                            .saturating_add(removed_type_indexes);
                    }
                }
            }
        }
