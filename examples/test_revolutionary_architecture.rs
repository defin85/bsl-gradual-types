...
use bsl_gradual_types::ideal::presentation::{LspCompletionRequest, WebSearchRequest, CliAnalysisRequest, CliOutputFormat};
...
async fn main() -> anyhow::Result<()> {
    ...
    assert!(metrics.platform_types > 0, "Платформенные типы должны быть загружены");
    ...
    assert!(health.overall_score > 0.8, "Оценка здоровья должна быть высокой");
    ...
    let completion_response = lsp_interface.handle_completion_request(completion_request).await?;
    println!("   - Автодополнение: {} элементов", completion_response.items.len());
    ...
    if let Some(hover_response) = lsp_interface.handle_hover_request(hover_request).await? {
        println!("   - Hover: {} элементов контента", hover_response.contents.len());
    }
    ...
    println!("   - Среднее время: {:.2}ms", perf_metrics.average_response_time_ms);
    ...
    println!("   - Иерархия: {} категорий", hierarchy_response.categories.len());
    println!("   - Всего типов в иерархии: {}", hierarchy_response.total_types);
    
    // В тестовом режиме категории могут быть пустыми (заглушки в Application Layer)
    println!("   - Категории: {} (в тестовом режиме могут быть пустыми)", hierarchy_response.categories.len());
    ...
    println!("   - Поиск 'массив': {} результатов", search_response.results.len());
    ...
    let analysis_response = cli_interface.handle_analysis_request(analysis_request).await?;
    println!("   - Анализ проекта:");
    println!("     • Файлов: {}", analysis_response.summary.total_files);
    println!("     • Функций: {}", analysis_response.summary.total_functions);
    println!("     • Переменных: {}", analysis_response.summary.total_variables);
    ...
    for i in 0..10 {
        ...
        let _response = lsp_interface.handle_completion_request(test_request).await?;
    }
    ...
    assert!(performance_time.as_millis() < 1000, "Производительность должна быть приемлемой");
    ...
    let all_layers_ready = final_health.components.iter()
    ...
    let passed_criteria = success_criteria.iter()
        .filter(|c| c.starts_with("✅"))
        .count();
    
    println!("\n🎯 РЕЗУЛЬТАТ: {}/{} критериев пройдено", passed_criteria, success_criteria.len());
