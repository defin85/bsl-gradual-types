...
use crate::core::dependency_graph::{
    DependencyNode, DependencyEdge, DependencyType, 
    TypeDependencyGraph, Scope, SourceLocation
};
            ...
            Expression::Call { function, args } => {
                ...
                if let Expression::Identifier(func_name) = &**function {
                    if let Some(target_node) = target {
                        ...
                        self.add_dependency(target_node.clone(), func_node.clone(), DependencyType::Expression);
                    ...
                    if let Expression::Identifier(obj_name) = &**object {
                        if let Some(target_node) = target {
                            ...
                            self.add_dependency(target_node.clone(), method_node, DependencyType::MethodCall);
                        }
                    }
                }
            ...
            Expression::MemberAccess { object, member } => {
                if let Expression::Identifier(obj_name) = &**object {
                    if let Some(target_node) = target {
                        ...
                        self.add_dependency(target_node.clone(), field_node, DependencyType::FieldAccess);
                    }
                }
...
mod tests {
    ...
    fn test_build_simple_graph() {
        ...
        assert!(deps.iter().any(|d| matches!(d, DependencyNode::Variable { name, .. } if name == "А")));
    }
    ...
    fn test_function_dependencies() {
        ...
        assert!(return_deps.iter().any(|d| matches!(d, DependencyNode::Function { name, .. } if name == "Факториал")));
    }
}