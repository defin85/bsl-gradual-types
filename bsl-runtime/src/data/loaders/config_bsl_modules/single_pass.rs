            ...
            for child in node.children(&mut cursor) {
                if child.kind() == "expression" || child.kind() == "method_call" {
                    ...
                    if track_returns {
                        ...
                        return_atoms
                            .push(infer_expr_atom_ts(&child, source).unwrap_or(ReturnAtom::Unknown));
                    }
                    walk_expr_ts(&child, source, calls, env);
                }
            }
...
fn infer_expr_atom_ts(node: &tree_sitter::Node, source: &str) -> Option<ReturnAtom> {
    ...
    match node.kind() {
        ...
        "method_call" | "call_expression" => parse_call_target_ts(node, source).map(ReturnAtom::Call),
        _ => None,
    }
}
...
fn seed_env_atoms_from_types(env: &HashMap<String, Vec<String>>) -> HashMap<String, Vec<ReturnAtom>> {
    let mut out: HashMap<String, Vec<ReturnAtom>> = HashMap::new();
    for (name, types) in env {
        let atoms = types.iter().cloned().map(ReturnAtom::Known).collect::<Vec<_>>();
