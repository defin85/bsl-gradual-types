...
**Распакованные файлы (.html, .st) НЕ должны попадать в git репозиторий!**

В `.gitignore` настроены правила для исключения:
- `examples/syntax_helper/**/*.html` - HTML файлы документации
- `examples/syntax_helper/**/*.st` - вспомогательные файлы
- `examples/syntax_helper/**/__categories__` - файлы категорий
- Распакованные папки: `rebuilt.shcntx_ru/`, `rebuilt.shlang_ru/`, `shcntx_ru/`, `shlang_ru/`

В репозитории хранятся **только ZIP архивы и .hbk файлы**.
