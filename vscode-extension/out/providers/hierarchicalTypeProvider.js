...
class HierarchicalTypeIndexProvider {
    ...
    async getRootCategories() {
        ...
        const items = [];
        const configPath = configHelper_1.BslAnalyzerConfig.configurationPath;
        const platformDocs = configHelper_1.BslAnalyzerConfig.platformDocsArchive;
        // Platform types group
        if (this.platformTypes.size > 0) {
            const platformGroup = new HierarchicalTypeItem(`🏢 Платформа 1С (${this.platformTypes.size} типов)`, vscode.TreeItemCollapsibleState.Collapsed, 'Платформа', 'platform-group');
            items.push(platformGroup);
        }
        else if (platformDocs) {
            // Platform docs configured but no cache
            const noPlatformItem = new HierarchicalTypeItem('⚠️ Кеш платформы не найден', vscode.TreeItemCollapsibleState.None, 'no-platform', 'empty');
            noPlatformItem.tooltip = 'Постройте индекс для создания кеша платформы';
            items.push(noPlatformItem);
        }
        else {
            // No platform docs configured
            const noPlatformDocsItem = new HierarchicalTypeItem('❌ Документация платформы не настроена', vscode.TreeItemCollapsibleState.None, 'no-platform-docs', 'empty');
        ...
        if (this.configTypes.size > 0) {
            const configGroup = new HierarchicalTypeItem(`🏗️ Конфигурация (${this.configTypes.size} типов)`, vscode.TreeItemCollapsibleState.Collapsed, 'Конфигурация', 'config-group');
            ...
            if (!projectId) {
                const invalidConfigItem = new HierarchicalTypeItem('❌ Неверная конфигурация (нет UUID)', vscode.TreeItemCollapsibleState.None, 'invalid-config', 'empty');
                invalidConfigItem.tooltip = 'Configuration.xml должен содержать валидный UUID';
                items.push(invalidConfigItem);
            }
            else {
                const noConfigItem = new HierarchicalTypeItem('⚠️ Кеш конфигурации не найден', vscode.TreeItemCollapsibleState.None, 'no-config', 'empty');
            ...
            // No config path set
            const noConfigPathItem = new HierarchicalTypeItem('ℹ️ Путь к конфигурации не указан', vscode.TreeItemCollapsibleState.None, 'no-config-path', 'empty');
