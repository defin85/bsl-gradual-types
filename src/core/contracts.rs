...
use super::types::{Contract, TypeResolution, Certainty, ResolutionResult, ConcreteType};
...
impl ContractGenerator {
    ...
    fn create_dynamic_check(&self, resolution: &TypeResolution) -> Contract {
        ...
        Contract {
            check_code,
            error_message: format!("Тип не может быть определён статически для: {:?}", resolution.metadata.file),
        }
    }
    ...
    fn generate_check_code(&self, resolution: &TypeResolution) -> String {
        match &resolution.result {
            ...
            ResolutionResult::Union(weighted_types) => {
                ...
                let concrete_types: Vec<ConcreteType> = weighted_types.iter()
                    .map(|wt| wt.type_.clone())
                    .collect();
                self.generate_union_check(&concrete_types)
            }
            ResolutionResult::Conditional(condition) => {
                self.generate_conditional_check(condition)
            }
            ResolutionResult::Dynamic => {
                self.generate_dynamic_check_code()
            }
            ResolutionResult::Contextual(contextual) => {
                format!("// Контекстный тип: {:?} в контексте {:?}", contextual.base_type, contextual.context)
            }
        }
    }
        ...
        match concrete_type {
            ...
            ConcreteType::Configuration(config) => {
                let type_name = match config.kind {
                    super::types::MetadataKind::Catalog => format!("СправочникСсылка.{}", config.name),
                    super::types::MetadataKind::Document => format!("ДокументСсылка.{}", config.name),
                    super::types::MetadataKind::Enum => format!("ПеречислениеСсылка.{}", config.name),
                    super::types::MetadataKind::Register => format!("РегистрСведенийКлючЗаписи.{}", config.name),
                    _ => config.name.clone(),
                };
    ...
    fn generate_union_check(&self, types: &[ConcreteType]) -> String {
        ...
        match self.mode {
            ContractMode::Assert => {
                format!(r#"// Проверка составного типа
                Если НЕ ({}) Тогда
                    ВызватьИсключение "Ошибка типа: ожидался один из типов {}";
                КонецЕсли;"#, checks, type_names.join(", "))
            }
