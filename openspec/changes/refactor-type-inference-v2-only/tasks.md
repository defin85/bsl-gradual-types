...
- [x] 3.3 Удалить/обновить устаревшие re-export обёртки (например, `backend/src/application/ast_to_ir/mod.rs`), чтобы не было скрытых обходов.

## 4. Удаление `bsl-semantic` из workspace
- [x] 4.1 Удалить crate `semantic/` из `[workspace.members]` и `[workspace.dependencies]`, обновить импорты.
- [x] 4.2 Удалить связанные упоминания в скриптах сборки (например, `scripts/build-all.sh`).

## 5. Тесты и документация
- [x] 5.1 Обновить/починить тесты, которые зависели от `bsl_semantic::AstToIrConverter` (перевести на v2 API).
- [x] 5.2 Обновить документацию v2 roadmap/архитектуры, где `bsl-semantic` описан как слой (переписать на v2‑only).

## 6. Валидация
- [x] 6.1 `cargo test -p bsl-analysis-v2 -p bsl-backend` проходит.
- [x] 6.2 `rg -n "bsl_semantic|bsl-semantic" -S .` не находит ссылок в коде (допускаются только исторические упоминания в archive docs, если явно помечены как архив).
- [x] 6.3 `openspec validate refactor-type-inference-v2-only --strict --no-interactive`.
