...
use bsl_agent::types::{
    BslDiagnosticsResponse, BslMembersResponse, BslSymbolSearchResponse, BslTypeAtPositionResponse,
    BuildInfoResponse, ContextExpandResponse, ContextPackResponse, JobStartResponse, JobStateDto,
    JobStatusResponse, UiUrlResponse, WorkspaceDocumentsSetResponse, WorkspaceListResponse,
...
#[tokio::test]
async fn stdio_flow_sensitive_tools_are_gated_by_flag() {
    let service = spawn_agent(&[]).await;

    let root = tempfile::TempDir::new().expect("root");
    std::fs::create_dir_all(root.path().join("src")).expect("mkdir");

    // Copy HBK files into a temp folder to avoid extracting large docs into the repo during tests.
    ...
    std::fs::write(root.path().join("src").join("test.bsl"), source).expect("write test.bsl");

    let diag_source = concat!(
        "Procedure Test()\n",
        "    x = Null;\n",
        "    x.Method();\n",
        "EndProcedure\n"
    );
    std::fs::write(root.path().join("src").join("diag.bsl"), diag_source).expect("write diag.bsl");
    ...
    .await;
    let status = wait_workspace_ready(&service, &open).await;
    assert!(status.ready);

    let root_id = &open.roots[0].root_id;

    // type_at_position: flow-sensitive OFF -> base type, ON -> narrowed type.
    {
        let start: JobStartResponse = call_tool(
            &service,
            "bsl_type_at_position_start",
            json!({
                "session_id": &open.session_id,
                "file": { "doc": { "root_id": root_id, "path": "src/test.bsl" } },
    ...
    // members: flow-sensitive OFF should not see Array method, ON should.
    {
        let members_source = concat!(
            "Процедура Test()\n",
            "    x = 0;\n",
            "    Если ТипЗнч(x) = Тип(\"Массив\") Тогда\n",
            "        x.\n",
        ...
        std::fs::write(root.path().join("src").join("members.bsl"), members_source)
            .expect("write members.bsl");
        ...
        let start: JobStartResponse = call_tool(
            ...
            json!({
                "session_id": &open.session_id,
                "file": { "doc": { "root_id": root_id, "path": "src/members.bsl" } },
    ...
    // diagnostics: flow-sensitive OFF should not include null-safety warning, ON should.
    {
        let start: JobStartResponse = call_tool(
            &service,
            "bsl_diagnostics_start",
            json!({
                "session_id": &open.session_id,
                "scope": { "kind": "file", "document": { "root_id": root_id, "path": "src/diag.bsl" } },
                "limit": 200
            }),
        )
        .await;
