...
async fn main() -> Result<()> {
    ...
    match cli.command {
            ...
            if !suggestions.is_empty() {
                ...
                for (i, suggestion) in suggestions.iter().enumerate() {
                    let priority_color = match suggestion.priority {
                        bsl_gradual_types::core::performance::OptimizationPriority::Critical => "red",
                        bsl_gradual_types::core::performance::OptimizationPriority::High => "yellow",
                        bsl_gradual_types::core::performance::OptimizationPriority::Medium => "blue",
        ...
        Commands::Profile { file, verbose, output } => {
            println!("{}", format!("🔬 Профилирование файла: {}", file.display()).cyan().bold());
                ...
                if !diagnostics.is_empty() {
                    ...
                    for diag in diagnostics.iter().take(5) {
                        let severity_color = match diag.severity {
                            bsl_gradual_types::core::type_checker::DiagnosticSeverity::Error => "red",
                            bsl_gradual_types::core::type_checker::DiagnosticSeverity::Warning => "yellow",
                            _ => "blue",
                        };
                        println!("  • {} [{}:{}] {}", 
        ...
        Commands::Compare { baseline, modified, iterations } => {
            ...
            let speedup = baseline_metrics.avg_time.as_nanos() as f64 / modified_metrics.avg_time.as_nanos() as f64;
            
            println!("\n{}", "📈 Результаты сравнения:".green().bold());
            println!("  Базовая версия: {:.2?} (среднее)", baseline_metrics.avg_time);
            println!("  Измененная версия: {:.2?} (среднее)", modified_metrics.avg_time);
            
            if speedup > 1.0 {
                println!("  🚀 Ускорение: {:.2}x", speedup.to_string().green().bold());
            } else if speedup < 1.0 {
                println!("  🐌 Замедление: {:.2}x", (1.0/speedup).to_string().red().bold());
            } else {
        ...
        Commands::Analyze { report } => {
            println!("{}", "🔍 Анализ отчета о производительности...".cyan().bold());
