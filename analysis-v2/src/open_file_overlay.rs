...
impl OpenFilesReturnOverlay {
    pub(crate) fn get(
        ...
        self.by_fn
            .get(&FnKey::new(owner_type_name, function_name))
    }
...
pub(crate) fn build_return_overlay_for_open_files(
    ...
    while changed && iterations < 64 {
        ...
        for key in &keys {
            ...
            let new_domain = evaluate_return_domain(facts, &domains);
        ...
        match stmt {
            Statement::Assignment { target, value, .. } => {
                if let Expression::Identifier { name, .. } = target {
                    ...
                    let atom = infer_expr_atom(value, owner_type_name, common_module_names_lower, facts);
            ...
            | Statement::ForEach { body, .. } => {
                collect_facts_from_statements(body, owner_type_name, common_module_names_lower, facts);
            }
            Statement::Try {
                try_body,
                except_body,
...
fn infer_expr_atom(
    ...
    match expr {
        ...
        Expression::Call { function, .. } => infer_call_atom(
            function,
            owner_type_name,
            common_module_names_lower,
            facts,
...
fn evaluate_return_domain(
    ...
    for atom in &facts.returns {
        eval_atom(atom, facts, known_domains, &mut domain);
    }
...
pub(crate) fn module_owner_key_from_file_path(file_path: &str) -> Option<String> {
    ...
    match location.module_type {
        ...
        ModuleType::ManagerModule { owner_type } => manager_owner_key_from_xml_owner(&owner_type),
        _ => None,
    }
}

fn manager_owner_key_from_xml_owner(owner_type: &str) -> Option<String> {
    let (xml_collection, object_name) = owner_type.split_once('.')?;
    let info = lookup_global_collection(xml_collection)?;
    Some(format!("{}.{}", info.item_manager_type, object_name))
}
