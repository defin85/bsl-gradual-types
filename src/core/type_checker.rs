...
use crate::core::dependency_graph::{Scope, TypeDependencyGraph};
...
impl TypeChecker {
    ...
    pub fn check(mut self, program: &Program) -> (TypeContext, Vec<TypeDiagnostic>) {
        ...
        self.interprocedural_analyzer = Some(InterproceduralAnalyzer::new(
            call_graph,
            self.context.clone(),
        ));
    ...
    fn infer_expression_type(&mut self, expr: &Expression) -> TypeResolution {
        match expr {
            ...
            Expression::Array(elements) => {
                ...
                let _element_types: Vec<_> = elements
                    .iter()
    ...
    fn types_compatible(&self, type1: &TypeResolution, type2: &TypeResolution) -> bool {
        ...
        if matches!(type1.certainty, Certainty::Unknown)
            || matches!(type2.certainty, Certainty::Unknown)
        {
            return true;
        }
    ...
    fn enter_scope(&mut self, scope: Scope) {
        self.context
            .scope_stack
            .push(self.context.current_scope.clone());
...
mod tests {
    ...
    fn test_simple_type_inference() {
        ...
        use crate::core::standard_types::{is_boolean, is_number, is_string};
