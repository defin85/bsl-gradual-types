...
async fn main() -> Result<()> {
    ...
    println!("   • Максимальная глубина: {}", hierarchy.statistics.max_depth);
    ...
    for category in &hierarchy.root_categories {
        ...
        println!("      └─ Методов: {}", category.statistics.total_methods_count);
        println!("      └─ Свойств: {}", category.statistics.total_properties_count);
    }
    ...
    if let Some(type_details) = doc_system.get_type_details("ТаблицаЗначений").await? {
        ...
        if !type_details.properties.is_empty() {
            ...
            for property in &type_details.properties {
                println!("      📋 {} / {}", property.russian_name, property.english_name);
            }
        }
    } else {
