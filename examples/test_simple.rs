...
fn main() {
    ...
    match parser.parse_with_configuration_guide() {
        Ok(resolutions) => {
            ...
            // Проверка регистра
            if let Some(metadata) = parser.get_discovered_metadata("РегистрыСведений.ТестовыйРегистрСведений") {
                ...
                let names: Vec<&str> = metadata.attributes.iter().map(|a| a.name.as_str()).collect();
