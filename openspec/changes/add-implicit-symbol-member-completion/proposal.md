...
- Зафиксировать детерминированный provider chain для `FormModule.Объект`:
...
- Зафиксировать, что intrinsic-слой только дополняет lookup, не переопределяя facet metadata.
