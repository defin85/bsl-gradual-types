...
impl CategoryHierarchyParser {
    ...
    pub fn parse_hierarchy(&mut self) -> Result<CategoryHierarchy> {
        ...
        println!("✅ Парсинг завершён: {} категорий, {} типов", 
    ...
    fn find_root_html_files(&self) -> Result<Vec<String>> {
        ...
        if !objects_dir.exists() {
            return Err(anyhow::anyhow!("Objects directory not found: {}", self.objects_path));
        }
    ...
    fn parse_html_file(&mut self, file_path: &str) -> Result<HtmlContent> {
        ...
        let description = desc_regex.captures(&content)
            .and_then(|caps| caps.get(1))
            .map(|m| {
                // Убираем HTML теги и лишние пробелы
                let text = m.as_str();
                let clean_text = Regex::new(r"<[^>]+>").unwrap()
                    .replace_all(text, " ");
        ...
        // Кешируем результат
        self.html_cache.insert(file_path.to_string(), html_content.clone());
